...
describe("Integration Tests", () => {
  let client: AptosClient,
    ...
    swapClient: HippoSwapClient,
    walletClient: HippoWalletClient;
  let app: App;
  it("create app", async () => {
    ...
    const faucetClient = new FaucetClient(
      HIPPO_CONF.fullNodeUrl,
      HIPPO_CONF.faucetUrl
    );
  ...
  it("faucet mint coins for account", async () => {
    await sendPayloadTx(
      client,
      account,
      await walletClient.makeFaucetMintToPayload(100, "BTC")
    );
    await sendPayloadTx(
      client,
      account,
      await walletClient.makeFaucetMintToPayload(1000, "USDT")
    );
    await sendPayloadTx(
      client,
      account,
      await walletClient.makeFaucetMintToPayload(1000, "USDC")
    );
    await sendPayloadTx(
      client,
      account,
      await walletClient.makeFaucetMintToPayload(1000, "DAI")
    );
