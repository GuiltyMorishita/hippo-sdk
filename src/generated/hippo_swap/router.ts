...
export function add_liquidity_route_(
  ...
  if ($.copy(pool_type).eq($.copy(POOL_TYPE_CONSTANT_PRODUCT))) {
    [temp$7, temp$8, temp$9] = Cp_swap.add_liquidity_(
      signer,
      $.copy(amount_x),
      $.copy(amount_y),
      $c,
    ...
    if ($.copy(pool_type).eq($.copy(POOL_TYPE_STABLE_CURVE))) {
      [temp$4, temp$5, temp$6] = Stable_curve_swap.add_liquidity_(
        signer,
        $.copy(amount_x),
        $.copy(amount_y),
        $c,
      ...
      if ($.copy(pool_type).eq($.copy(POOL_TYPE_PIECEWISE))) {
        [temp$1, temp$2, temp$3] = Piece_swap.add_liquidity_(
          signer,
          $.copy(amount_x),
          $.copy(amount_y),
          $c,
...
export function get_intermediate_output_(
  ...
  if ($.copy(pool_type).eq($.copy(POOL_TYPE_CONSTANT_PRODUCT))) {
    if (is_x_to_y) {
      [x_out, y_out] = Cp_swap.swap_x_to_exact_y_direct_(x_in, $c, [
        $p[0],
        $p[1],
      ]);
      ...
      [y_out__1, x_out__2] = Cp_swap.swap_y_to_exact_x_direct_(x_in, $c, [
        $p[1],
        $p[0],
      ]);
    ...
    if ($.copy(pool_type).eq($.copy(POOL_TYPE_STABLE_CURVE))) {
      if (is_x_to_y) {
        [zero, zero2, y_out__4] = Stable_curve_swap.swap_x_to_exact_y_direct_(
          x_in,
          $c,
          [$p[0], $p[1]]
        );
        ...
        [zero__5, y_out__6, zero2__7] =
          Stable_curve_swap.swap_y_to_exact_x_direct_(x_in, $c, [$p[1], $p[0]]);
...
export function remove_liquidity_route_(
  ...
  if ($.copy(pool_type).eq($.copy(POOL_TYPE_CONSTANT_PRODUCT))) {
    ...
    if ($.copy(pool_type).eq($.copy(POOL_TYPE_STABLE_CURVE))) {
      ...
      if ($.copy(pool_type).eq($.copy(POOL_TYPE_PIECEWISE))) {
        [temp$1, temp$2] = Piece_swap.remove_liquidity_(
          signer,
          $.copy(liquidity),
          $c,
          [$p[0], $p[1]]
...
export function three_step_route_(
  ...
  coin_y = get_intermediate_output_(
  ...
  coin_z = get_intermediate_output_(
  ...
  coin_a = get_intermediate_output_(
