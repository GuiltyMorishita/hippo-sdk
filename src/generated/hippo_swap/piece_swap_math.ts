...
export function compute_initialization_constants$ (
  ...
  let k2, m, n, xa, xb;
  m = math$_.sqrt$(div_w$($.copy(k), $.copy(w1_numerator), $.copy(w1_denominator), $c), $c);
  xa = math$_.sqrt$(div_w$($.copy(k), $.copy(w2_numerator), $.copy(w2_denominator), $c), $c).sub($.copy(m));
...
export function get_swap_x_to_y_out$ (
  ...
  max_x_y = math$_.max$($.copy(current_x), $.copy(current_y), $c);
  numerator = u128("1");
  denominator = u128("1");
...
export function get_swap_x_to_y_out_preprocessed_inner$ (
  ...
  if (compare_fraction$($.copy(current_x), $.copy(current_y), $.copy(xa), $.copy(xb), $c)) {
    ...
    if ($.copy(p_new_xF).gt($.copy(p_xa))) {
      ...
      temp$3 = math$_.min$($.copy(p_output_y), $.copy(p_output_y_max), $c);
    }
    else{
    ...
    if (compare_fraction$($.copy(current_x), $.copy(current_y), $.copy(xb), $.copy(xa), $c)) {
      ...
      if ($.copy(p_new_xF__11).gt($.copy(p_xb))) {
        ...
        temp$20 = math$_.min$($.copy(p_output_y__16), $.copy(p_output_y_max__12), $c);
      }
      else{
...
export function solve_F_middle$ (
  ...
  numerator = math$_.sqrt$($.copy(b).mul($.copy(b)).add(u128("4").mul($.copy(xy)).mul($.copy(k).sub($.copy(m).mul($.copy(m))))), $c).sub($.copy(b));
...
export function solve_F_upper_left$ (
  ...
  xy = $.copy(x).mul($.copy(y));
  numerator = $.copy(xn).add(math$_.sqrt$($.copy(xn).mul($.copy(xn)).add(u128("4").mul($.copy(xy)).mul($.copy(k2))), $c));
  denominator = u128("2").mul($.copy(xy));
  xF = mul_w$($.copy(x), $.copy(numerator), $.copy(denominator), $c);
