...
export function add_liquidity_(
  ...
  Stable_curve_swap.add_liquidity_(
    sender,
    $.copy(amount_x),
    $.copy(amount_y),
    $c,
...
export function create_new_pool_(
  ...
  future_time = $.copy(block_timestamp).add(
    u64("24").mul(u64("3600")).mul($.copy(MICRO_CONVERSION_FACTOR))
  );
  decimals = Math.max_(
...
export function mock_create_pair_and_add_liquidity_(
  ...
  future_A_time = $.copy(initial_A_time).add(
    u64("24").mul(u64("3600")).mul($.copy(MICRO_CONVERSION_FACTOR))
  );
  decimals = Math.max_(
    u128(Stdlib.Coin.decimals_($c, [$p[0]])),
  ...
  [unused_x, unused_y, some_lp] = Stable_curve_swap.add_liquidity_direct_(
    some_x,
    some_y,
    $c,
    [$p[0], $p[1]]
...
export function remove_liquidity_(
  ...
  Stable_curve_swap.remove_liquidity_(
    sender,
    $.copy(liquidity),
    $.copy(min_amount_x),
    $.copy(min_amount_y),
...
export function swap_(
  ...
  if ($.copy(x_in).gt(u64("0"))) {
    [, , out_amount] = Stable_curve_swap.swap_x_to_exact_y_(
      sender,
      $.copy(x_in),
      $.copy(addr),
      $c,
    ...
    [, out_amount__4] = Stable_curve_swap.swap_y_to_exact_x_(
      sender,
      $.copy(y_in),
      $.copy(addr),
      $c,
...
export class App {
  ...
  payload_remove_liquidity(
    ...
    | TxnBuilderTypes.TransactionPayloadEntryFunction
    | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_remove_liquidity(
      liquidity,
      min_amount_x,
      min_amount_y,
      $p,
  ...
  async remove_liquidity(
    ...
    const payload = buildPayload_remove_liquidity(
      liquidity,
      min_amount_x,
      min_amount_y,
      $p,
