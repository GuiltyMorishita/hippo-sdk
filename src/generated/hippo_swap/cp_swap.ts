...
export class TokenPairMetadata {
  ...
  static TokenPairMetadataParser(
    data: any,
    typeTag: TypeTag,
    repo: AptosParserRepo
  ): TokenPairMetadata {
...
export function add_liquidity_(
  ...
  if (temp$1) {
    ...
    amount_y_optimal = Cp_swap_utils.quote_(
      $.copy(amount_x),
      $.copy(reserve_x),
      $.copy(reserve_y),
      $c
    ...
    if ($.copy(amount_y_optimal).le($.copy(amount_y))) {
      ...
      amount_x_optimal = Cp_swap_utils.quote_(
        $.copy(amount_y),
        $.copy(reserve_y),
        $.copy(reserve_x),
        $c
  ...
  deposit_x_(Stdlib.Coin.withdraw_(sender, $.copy(a_x), $c, [$p[0]]), $c, [
    $p[0],
    $p[1],
  ]);
  deposit_y_(Stdlib.Coin.withdraw_(sender, $.copy(a_y), $c, [$p[1]]), $c, [
    $p[0],
    $p[1],
  ]);
  sender_addr = Stdlib.Signer.address_of_(sender, $c);
  lp = mint_($c, [$p[0], $p[1]]);
  lp_amount = Stdlib.Coin.value_(lp, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  check_coin_store_(sender, $c, [new SimpleStructTag(LPToken, [$p[0], $p[1]])]);
  Stdlib.Coin.deposit_($.copy(sender_addr), lp, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
...
export function add_liquidity_direct_(
  ...
  left_x = Stdlib.Coin.extract_(x, $.copy(amount_x).sub($.copy(a_x)), $c, [
    $p[0],
  ]);
  left_y = Stdlib.Coin.extract_(y, $.copy(amount_y).sub($.copy(a_y)), $c, [
    $p[1],
  ]);
...
export function burn_(
  ...
  [balance_x, balance_y] = token_balances_metadata_(metadata, $c, [
    $p[0],
    $p[1],
  ]);
  liquidity = Stdlib.Coin.value_(metadata.lp, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  mint_fee_(
    $.copy(reserves.reserve_x),
    $.copy(reserves.reserve_y),
    metadata,
    $c,
    [$p[0], $p[1]]
  );
  ...
  [balance_x__2, balance_y__3] = token_balances_metadata_(metadata, $c, [
    $p[0],
    $p[1],
  ]);
  update_($.copy(balance_x__2), $.copy(balance_y__3), reserves, $c, [
    $p[0],
    $p[1],
  ]);
  if ($.copy(metadata.fee_on)) {
    metadata.k_last = Safe_math.mul_(
      u128($.copy(reserves.reserve_x)),
      u128($.copy(reserves.reserve_y)),
      $c
    );
  } else {
  }
...
export function burn_lp_(
  ...
  let coins;
  if (
    !Stdlib.Coin.value_(metadata.lp, $c, [
      new SimpleStructTag(LPToken, [$p[0], $p[1]]),
    ]).ge($.copy(amount))
  ) {
    throw $.abortCode($.copy(ERROR_INSUFFICIENT_LIQUIDITY));
  }
  coins = Stdlib.Coin.extract_(metadata.lp, $.copy(amount), $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  Stdlib.Coin.burn_(coins, metadata.burn_cap, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  return;
}
...
export function create_token_pair_(
  ...
  if (
    !!$c.exists(
      new SimpleStructTag(TokenPairReserve, [$p[1], $p[0]]),
      $.copy(sender_addr)
    )
  ) {
    throw $.abortCode($.copy(ERROR_ALREADY_INITIALIZED));
...
export function get_reserves_(
  ...
  return [
    $.copy(reserve.reserve_x),
    $.copy(reserve.reserve_y),
    $.copy(reserve.block_timestamp_last),
  ];
}
...
export function lp_balance_(
  ...
  return Stdlib.Coin.balance_($.copy(addr), $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
}
...
export function mint_(
  ...
  amount_x = Safe_math.sub_(
    u128($.copy(balance_x)),
    u128($.copy(reserves.reserve_x)),
    $c
  );
  amount_y = Safe_math.sub_(
    u128($.copy(balance_y)),
    u128($.copy(reserves.reserve_y)),
    $c
  );
  ...
  if ($.copy(total_supply).eq(u128("0"))) {
    ...
    temp$1 = Math.min_(
      Safe_math.div_(
        Safe_math.mul_($.copy(amount_x), $.copy(total_supply), $c),
        u128($.copy(reserves.reserve_x)),
        $c
      ),
      Safe_math.div_(
        Safe_math.mul_($.copy(amount_y), $.copy(total_supply), $c),
        u128($.copy(reserves.reserve_y)),
        $c
      ),
      $c
...
export function mint_fee_(
  ...
  if ($.copy(metadata.fee_on)) {
    if ($.copy(metadata.k_last).neq(u128("0"))) {
      root_k = Math.sqrt_(
        Safe_math.mul_(u128($.copy(reservex)), u128($.copy(reservey)), $c),
        $c
      );
      ...
      if ($.copy(root_k).gt($.copy(root_k_last))) {
        ...
        numerator = Safe_math.mul_(
          $.copy(total_supply),
          Safe_math.sub_($.copy(root_k), $.copy(root_k_last), $c),
          $c
        );
        denominator = Safe_math.add_(
          $.copy(root_k_last),
          Safe_math.mul_($.copy(root_k), u128("5"), $c),
          $c
        );
        liquidity = u64(
          Safe_math.div_($.copy(numerator), $.copy(denominator), $c)
        );
        mint_lp_to_(
          $.copy(metadata.fee_to),
          $.copy(liquidity),
          metadata.mint_cap,
          $c,
          [$p[0], $p[1]]
...
export function mint_lp_(
  ...
  return Stdlib.Coin.mint_($.copy(amount), mint_cap, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
}
...
export function mint_lp_to_(
  ...
  let coins;
  coins = Stdlib.Coin.mint_($.copy(amount), mint_cap, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  Stdlib.Coin.deposit_($.copy(to), coins, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  return;
}
...
export function quote_x_to_y_after_fees_(
  ...
  return Cp_swap_utils.get_amount_out_(
    $.copy(amount_x_in),
    $.copy(x_balance),
    $.copy(y_balance),
    $c
...
export function register_account_(
  ...
  Stdlib.Coin.register_(sender, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  return;
}
...
export function remove_liquidity_(
  ...
  coins = Stdlib.Coin.withdraw_(sender, $.copy(liquidity), $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  [coins_x, coins_y] = remove_liquidity_direct_(
    coins,
    $.copy(amount_x_min),
    $.copy(amount_y_min),
    $c,
  ...
  Stdlib.Coin.deposit_(Stdlib.Signer.address_of_(sender, $c), coins_x, $c, [
    $p[0],
  ]);
  Stdlib.Coin.deposit_(Stdlib.Signer.address_of_(sender, $c), coins_y, $c, [
    $p[1],
  ]);
...
export function swap_(
  ...
  if ($.copy(amount_x_out).gt(u64("0"))) {
    Stdlib.Coin.merge_(
      coins_x_out,
      extract_x_($.copy(amount_x_out), metadata, $c, [$p[0], $p[1]]),
      $c,
      [$p[0]]
    );
  } else {
  ...
  if ($.copy(amount_y_out).gt(u64("0"))) {
    Stdlib.Coin.merge_(
      coins_y_out,
      extract_y_($.copy(amount_y_out), metadata, $c, [$p[0], $p[1]]),
      $c,
      [$p[1]]
    );
  } else {
  ...
  if (
    $.copy(balance_x).gt($.copy(reserves.reserve_x).sub($.copy(amount_x_out)))
  ) {
    temp$3 = $.copy(balance_x).sub(
      $.copy(reserves.reserve_x).sub($.copy(amount_x_out))
    );
  } else {
    temp$3 = u64("0");
  }
  amount_x_in = temp$3;
  if (
    $.copy(balance_y).gt($.copy(reserves.reserve_y).sub($.copy(amount_y_out)))
  ) {
    temp$4 = $.copy(balance_y).sub(
      $.copy(reserves.reserve_y).sub($.copy(amount_y_out))
    );
  } else {
    temp$4 = u64("0");
  }
  ...
  if (
    !Safe_math.mul_(
      $.copy(balance_x_adjusted),
      $.copy(balance_y_adjusted),
      $c
    ).ge($.copy(k))
  ) {
...
export function swap_x_to_exact_y_(
  ...
  [coins_x_out, coins_y_out] = swap_x_to_exact_y_direct_(coins, $c, [
    $p[0],
    $p[1],
  ]);
...
export function swap_x_to_exact_y_direct_(
  ...
  amount_out = Cp_swap_utils.get_amount_out_(
    $.copy(amount_in),
    $.copy(rin),
    $.copy(rout),
    $c
  );
  [coins_x_out, coins_y_out] = swap_(u64("0"), $.copy(amount_out), $c, [
    $p[0],
    $p[1],
  ]);
...
export function swap_y_to_exact_x_(
  ...
  [coins_x_out, coins_y_out] = swap_y_to_exact_x_direct_(coins, $c, [
    $p[0],
    $p[1],
  ]);
...
export function swap_y_to_exact_x_direct_(
  ...
  [coins_x_out, coins_y_out] = swap_($.copy(amount_out), u64("0"), $c, [
    $p[0],
    $p[1],
  ]);
...
export function token_balances_metadata_(
  ...
  return [
    Stdlib.Coin.value_(metadata.balance_x, $c, [$p[0]]),
    Stdlib.Coin.value_(metadata.balance_y, $c, [$p[1]]),
  ];
}
...
export function total_lp_supply_(
  ...
  let temp$1;
  temp$1 = Stdlib.Coin.supply_($c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  return Stdlib.Option.get_with_default_(temp$1, u128("0"), $c, [
    AtomicTypeTag.U128,
  ]);
}
...
export function tranfer_lp_coin_in_(
  ...
  Stdlib.Coin.merge_(metadata.lp, coins, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  return;
}
