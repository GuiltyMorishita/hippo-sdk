...
export class PieceSwapPoolInfo {
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.protocol_fee_share_per_thousand = proto[
  ...
  static PieceSwapPoolInfoParser(
    data: any,
    typeTag: TypeTag,
    repo: AptosParserRepo
  ): PieceSwapPoolInfo {
...
export function add_liquidity_(
  ...
  current_x = u128(Stdlib.Coin.value_(pool.reserve_x, $c, [$p[0]])).mul(
    u128($.copy(pool.x_deci_mult))
  );
  current_y = u128(Stdlib.Coin.value_(pool.reserve_y, $c, [$p[1]])).mul(
    u128($.copy(pool.y_deci_mult))
  );
  [opt_amt_x, opt_amt_y, opt_lp] =
    Piece_swap_math.get_add_liquidity_actual_amount_(
      $.copy(current_x),
      $.copy(current_y),
...
export function add_liquidity_direct_(
  ...
  if ($.copy(opt_lp).eq(u128("0"))) {
    return [
      coin_x,
      coin_y,
      Stdlib.Coin.zero_($c, [new SimpleStructTag(LPToken, [$p[0], $p[1]])]),
    ];
  } else {
  }
  actual_add_x = u64($.copy(opt_amt_x).div(u128($.copy(pool.x_deci_mult))));
  actual_add_y = u64($.copy(opt_amt_y).div(u128($.copy(pool.y_deci_mult))));
  actual_add_x_coin = Stdlib.Coin.extract_(coin_x, $.copy(actual_add_x), $c, [
    $p[0],
  ]);
  actual_add_y_coin = Stdlib.Coin.extract_(coin_y, $.copy(actual_add_y), $c, [
    $p[1],
  ]);
...
export function burn_direct_(
  ...
  let amount;
  amount = Stdlib.Coin.value_(lp, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  Stdlib.Coin.burn_(lp, pool.lp_burn_cap, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
...
export function burn_from_(
  ...
  let coin_to_burn;
  coin_to_burn = Stdlib.Coin.withdraw_(from, $.copy(amount), $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
...
export function create_new_pool_(
  ...
  if (
    !!$c.exists(
      new SimpleStructTag(PieceSwapPoolInfo, [$p[1], $p[0]]),
      $.copy(admin_addr)
    )
  ) {
    throw $.abortCode($.copy(ERROR_ALREADY_INITIALIZED));
  ...
  if ($.copy(x_decimals).gt($.copy(y_decimals))) {
    [temp$3, temp$4] = [
      u128("1"),
      Math.pow_(u128("10"), u8($.copy(x_decimals).sub($.copy(y_decimals))), $c),
    ];
  } else {
    if ($.copy(y_decimals).gt($.copy(x_decimals))) {
      [temp$1, temp$2] = [
        Math.pow_(
          u128("10"),
          u8($.copy(y_decimals).sub($.copy(x_decimals))),
          $c
        ),
        u128("1"),
      ];
    ...
    new PieceSwapPoolInfo(
      {
        ...
        protocol_fee_share_per_thousand: $.copy(
          protocol_fee_share_per_thousand
        ),
...
export function mint_direct_(
  ...
  let lp_coin;
  lp_coin = Stdlib.Coin.mint_($.copy(amount), pool.lp_mint_cap, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
...
export function mint_to_(
  ...
  check_and_deposit_(to, lp_coin, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  return;
}
...
export function quote_x_to_y_after_fees_(
  ...
  total_fees = $.copy(actual_out_y)
    .mul($.copy(pool.swap_fee_per_million))
    .div(u64("1000000"));
...
export function remove_liquidity_(
  ...
  removed_x = Stdlib.Coin.extract_(
    pool.reserve_x,
    $.copy(actual_remove_x),
    $c,
    [$p[0]]
  );
  removed_y = Stdlib.Coin.extract_(
    pool.reserve_y,
    $.copy(actual_remove_y),
    $c,
    [$p[1]]
  );
...
export function remove_liquidity_direct_(
  ...
  remove_lp_amt = Stdlib.Coin.value_(remove_lp, $c, [
    new SimpleStructTag(LPToken, [$p[0], $p[1]]),
  ]);
  [opt_amt_x, opt_amt_y] = Piece_swap_math.get_remove_liquidity_amounts_(
    $.copy(current_x),
...
export function swap_x_to_y_direct_(
  ...
  total_fees = $.copy(actual_out_y)
    .mul($.copy(pool.swap_fee_per_million))
    .div(u64("1000000"));
  protocol_fees = $.copy(total_fees)
    .mul($.copy(pool.protocol_fee_share_per_thousand))
    .div(u64("1000"));
  out_y_after_fees = $.copy(actual_out_y).sub($.copy(total_fees));
  coin_y = Stdlib.Coin.extract_(pool.reserve_y, $.copy(out_y_after_fees), $c, [
    $p[1],
  ]);
  protocol_fee_y = Stdlib.Coin.extract_(
    pool.reserve_y,
    $.copy(protocol_fees),
    $c,
    [$p[1]]
...
export function swap_y_to_x_direct_(
  ...
  protocol_fees = $.copy(total_fees)
    ...
    .div(u64("1000"));
  out_x_after_fees = $.copy(actual_out_x).sub($.copy(total_fees));
  protocol_fee_x = Stdlib.Coin.extract_(
    pool.reserve_x,
    $.copy(protocol_fees),
    $c,
    [$p[0]]
  ...
  coin_x = Stdlib.Coin.extract_(pool.reserve_x, $.copy(out_x_after_fees), $c, [
    $p[0],
  ]);
  return coin_x;
}
