...
export function create_new_pool_ (
  ...
  [lp_burn_cap, lp_freeze_cap, lp_mint_cap] = Stdlib.Coin.initialize_(admin, Stdlib.String.utf8_($.copy(lp_name), $c), Stdlib.String.utf8_($.copy(lp_symbol), $c), $.copy(lp_decimals), true, $c, [new SimpleStructTag(LPToken, [$p[0], $p[1]])]);
  ...
  $c.move_to(new SimpleStructTag(PieceSwapPoolInfo, [$p[0], $p[1]]), admin, new PieceSwapPoolInfo({ reserve_x: Stdlib.Coin.zero_($c, [$p[0]]), reserve_y: Stdlib.Coin.zero_($c, [$p[1]]), lp_amt: u64("0"), lp_mint_cap: $.copy(lp_mint_cap), lp_burn_cap: $.copy(lp_burn_cap), lp_freeze_cap: $.copy(lp_freeze_cap), K: $.copy(k), K2: $.copy(k2), Xa: $.copy(xa), Xb: $.copy(xb), m: $.copy(m), n: $.copy(n), x_deci_mult: u64($.copy(x_deci_mult)), y_deci_mult: u64($.copy(y_deci_mult)), swap_fee_per_million: $.copy(swap_fee_per_million), protocol_fee_share_per_thousand: $.copy(protocol_fee_share_per_thousand), protocol_fee_x: Stdlib.Coin.zero_($c, [$p[0]]), protocol_fee_y: Stdlib.Coin.zero_($c, [$p[1]]) }, new SimpleStructTag(PieceSwapPoolInfo, [$p[0], $p[1]])));
