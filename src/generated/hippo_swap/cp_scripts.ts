...
export function mock_create_pair_and_add_liquidity_(
  ...
  create_new_pool_(admin, Stdlib.Signer.address_of_(admin, $c), false, $.copy(symbol), $.copy(symbol), [], [], $c, [
    $p[0],
    $p[1]
  ]);
...
export function swap_script_(
  ...
  if ($.copy(x_in).gt(u64('0'))) {
    ...
    if ($.copy(y_in).gt(u64('0'))) {
      x_out = Cp_swap.swap_y_to_exact_x_(sender, $.copy(y_in), Stdlib.Signer.address_of_(sender, $c), $c, [
        $p[0],
        $p[1]
      ]);
