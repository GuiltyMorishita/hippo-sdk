...
export function borrow_(tree: CritBitTree, key: U128, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): any {
...
export function borrow_closest_outer_node_(
  ...
  if (is_outer_node_($.copy(tree.root), $c)) {
    return Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(tree.root), $c), $c, [
      new SimpleStructTag(OuterNode, [$p[0]])
    ]);
  } else {
  }
  ...
  while (true) {
    ...
    if (is_outer_node_($.copy(child_index), $c)) {
      return Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(child_index), $c), $c, [
        new SimpleStructTag(OuterNode, [$p[0]])
      ]);
    } else {
    }
...
export function borrow_closest_outer_node_mut_(
  ...
  let temp$1, child_index, node;
  if (is_outer_node_($.copy(tree.root), $c)) {
    return Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(tree.root), $c), $c, [
      new SimpleStructTag(OuterNode, [$p[0]])
    ]);
  } else {
  }
  ...
  while (true) {
    ...
    child_index = temp$1;
    if (is_outer_node_($.copy(child_index), $c)) {
      return Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(child_index), $c), $c, [
        new SimpleStructTag(OuterNode, [$p[0]])
      ]);
    } else {
    }
...
export function borrow_mut_(tree: CritBitTree, key: U128, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): any {
...
export function check_length_(length: U64, $c: AptosDataCache): void {
...
export function crit_bit_(s1: U128, s2: U128, $c: AptosDataCache): U8 {
  let l, m, s, u, x;
...
export function destroy_empty_(tree: CritBitTree, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): void {
  if (!is_empty_(tree, $c, [$p[0]])) {
...
export function empty_($c: AptosDataCache, $p: TypeTag[] /* <V>*/): CritBitTree {
  return new CritBitTree(
    {
      root: u64('0'),
      inner_nodes: Stdlib.Vector.empty_($c, [new SimpleStructTag(InnerNode)]),
      outer_nodes: Stdlib.Vector.empty_($c, [new SimpleStructTag(OuterNode, [$p[0]])])
    },
    new SimpleStructTag(CritBitTree, [$p[0]])
  );
}

export function has_key_(tree: CritBitTree, key: U128, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): boolean {
  if (is_empty_(tree, $c, [$p[0]])) {
    return false;
  } else {
  }
...
export function insert_(tree: CritBitTree, key: U128, value: any, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): void {
  let length;
...
export function insert_above_(
  ...
  let node, node_index;
  node_index = $.copy(
    Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(search_parent_index), $c, [new SimpleStructTag(InnerNode)])
      .parent_index
  );
  while (true) {
    if ($.copy(node_index).eq($.copy(ROOT))) {
      return insert_above_root_(
        tree,
        $.copy(key),
        value,
        $.copy(n_outer_nodes),
...
export function insert_above_root_(
  ...
  Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(old_root_index), $c, [
    new SimpleStructTag(InnerNode)
  ]).parent_index = $.copy(n_inner_nodes);
  tree.root = $.copy(n_inner_nodes);
  push_back_insert_nodes_(
    tree,
    $.copy(key),
    value,
    $.copy(n_inner_nodes),
    $.copy(critical_bit),
    $.copy(ROOT),
    is_set_($.copy(key), $.copy(critical_bit), $c),
    $.copy(old_root_index),
    outer_node_child_index_($.copy(n_outer_nodes), $c),
    $c,
    [$p[0]]
  );
  return;
...
export function insert_below_(
  ...
  let search_parent;
  search_parent = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(search_parent_index), $c, [
    new SimpleStructTag(InnerNode)
  ]);
  ...
  Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(search_index), $c), $c, [
    new SimpleStructTag(OuterNode, [$p[0]])
  ]).parent_index = $.copy(n_inner_nodes);
  push_back_insert_nodes_(
    tree,
    ...
    $.copy(n_inner_nodes),
    $.copy(critical_bit),
    $.copy(search_parent_index),
    $.copy(key).lt($.copy(search_key)),
    outer_node_child_index_($.copy(n_outer_nodes), $c),
    $.copy(search_index),
    $c,
    [$p[0]]
...
export function insert_below_walk_(
  ...
  review_node = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(review_node_index), $c, [
    new SimpleStructTag(InnerNode)
  ]);
  ...
  Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(walked_child_index), $c, [
    new SimpleStructTag(InnerNode)
  ]).parent_index = $.copy(n_inner_nodes);
  push_back_insert_nodes_(
    tree,
    ...
    $.copy(n_inner_nodes),
    $.copy(critical_bit),
    $.copy(review_node_index),
    is_set_($.copy(key), $.copy(critical_bit), $c),
    $.copy(walked_child_index),
    outer_node_child_index_($.copy(n_outer_nodes), $c),
    $c,
    [$p[0]]
  );
...
export function insert_empty_(
  ...
  Stdlib.Vector.push_back_(
    tree.outer_nodes,
    new OuterNode(
      { key: $.copy(key), value: value, parent_index: $.copy(ROOT) },
      new SimpleStructTag(OuterNode, [$p[0]])
    ),
    $c,
    [new SimpleStructTag(OuterNode, [$p[0]])]
...
export function insert_general_(
  ...
  let temp$1,
    ...
    search_index,
    search_key,
    search_parent_critical_bit,
    search_parent_index;
  n_inner_nodes = Stdlib.Vector.length_(tree.inner_nodes, $c, [new SimpleStructTag(InnerNode)]);
  [temp$1, temp$2] = [tree, $.copy(key)];
  [search_index, search_child_side, search_key, search_parent_index, search_parent_critical_bit] = search_outer_(
    temp$1,
    temp$2,
    $c,
    [$p[0]]
...
export function insert_singleton_(
  ...
  push_back_insert_nodes_(
    ...
    $.copy(critical_bit),
    $.copy(ROOT),
    $.copy(key).gt($.copy(outer_node.key)),
    outer_node_child_index_(u64('0'), $c),
    outer_node_child_index_(u64('1'), $c),
    $c,
    [$p[0]]
  );
  tree.root = u64('0');
  Stdlib.Vector.borrow_mut_(tree.outer_nodes, u64('0'), $c, [new SimpleStructTag(OuterNode, [$p[0]])]).parent_index =
    u64('0');
  return;
}

export function is_empty_(tree: CritBitTree, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): boolean {
  return Stdlib.Vector.is_empty_(tree.outer_nodes, $c, [new SimpleStructTag(OuterNode, [$p[0]])]);
}

export function is_outer_node_(child_field_index: U64, $c: AptosDataCache): boolean {
  return $.copy(child_field_index).shr($.copy(NODE_TYPE)).and($.copy(OUTER)).eq($.copy(OUTER));
}

export function is_set_(key: U128, bit_number: U8, $c: AptosDataCache): boolean {
  return $.copy(key).shr($.copy(bit_number)).and(u128('1')).eq(u128('1'));
}

export function length_(tree: CritBitTree, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): U64 {
  return Stdlib.Vector.length_(tree.outer_nodes, $c, [new SimpleStructTag(OuterNode, [$p[0]])]);
}

export function max_key_(tree: CritBitTree, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): U128 {
  if (!!is_empty_(tree, $c, [$p[0]])) {
    throw $.abortCode($.copy(E_LOOKUP_EMPTY));
  }
  return $.copy(
    Stdlib.Vector.borrow_(
      tree.outer_nodes,
      outer_node_vector_index_(max_node_child_index_(tree, $c, [$p[0]]), $c),
      $c,
      [new SimpleStructTag(OuterNode, [$p[0]])]
    ).key
  );
...
export function max_node_child_index_(tree: CritBitTree, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): U64 {
  ...
  while (true) {
    ...
    child_field_index = $.copy(
      Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_field_index), $c, [new SimpleStructTag(InnerNode)])
        .right_child_index
    );
  }
}

export function min_key_(tree: CritBitTree, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): U128 {
  if (!!is_empty_(tree, $c, [$p[0]])) {
    throw $.abortCode($.copy(E_LOOKUP_EMPTY));
  }
  return $.copy(
    Stdlib.Vector.borrow_(
      tree.outer_nodes,
      outer_node_vector_index_(min_node_child_index_(tree, $c, [$p[0]]), $c),
      $c,
      [new SimpleStructTag(OuterNode, [$p[0]])]
    ).key
  );
...
export function min_node_child_index_(tree: CritBitTree, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): U64 {
  let child_field_index;
  child_field_index = $.copy(tree.root);
  while (true) {
    if (is_outer_node_($.copy(child_field_index), $c)) {
...
export function outer_node_child_index_(vector_index: U64, $c: AptosDataCache): U64 {
  return $.copy(vector_index).or($.copy(OUTER).shl($.copy(NODE_TYPE)));
}

export function outer_node_vector_index_(child_field_index: U64, $c: AptosDataCache): U64 {
  return $.copy(child_field_index)
    .and($.copy(HI_64))
    .xor($.copy(OUTER).shl($.copy(NODE_TYPE)));
}

export function pop_(tree: CritBitTree, key: U128, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): any {
  let temp$1, length;
  if (!!is_empty_(tree, $c, [$p[0]])) {
...
export function pop_destroy_nodes_(
  ...
  let { value: value } = Stdlib.Vector.swap_remove_(
    tree.outer_nodes,
    outer_node_vector_index_($.copy(outer_index), $c),
    $c,
    [new SimpleStructTag(OuterNode, [$p[0]])]
  );
...
export function pop_general_(
  ...
  return pop_destroy_nodes_(tree, $.copy(search_parent_index), $.copy(search_index), $.copy(n_outer_nodes), $c, [
    $p[0]
  ]);
}

export function pop_singleton_(tree: CritBitTree, key: U128, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): any {
  if (
    !$.copy(Stdlib.Vector.borrow_(tree.outer_nodes, u64('0'), $c, [new SimpleStructTag(OuterNode, [$p[0]])]).key).eq(
      $.copy(key)
    )
  ) {
    throw $.abortCode($.copy(E_NOT_HAS_KEY));
...
export function pop_update_relationships_(
  ...
  if (is_outer_node_($.copy(sibling_index), $c)) {
    Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(sibling_index), $c), $c, [
      new SimpleStructTag(OuterNode, [$p[0]])
    ]).parent_index = $.copy(grandparent_index);
  } else {
    Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(sibling_index), $c, [
      new SimpleStructTag(InnerNode)
    ]).parent_index = $.copy(grandparent_index);
  }
  if ($.copy(grandparent_index).eq($.copy(ROOT))) {
    ...
    grandparent = Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(grandparent_index), $c, [
      new SimpleStructTag(InnerNode)
    ]);
...
export function push_back_insert_nodes_(
  ...
  Stdlib.Vector.push_back_(
    tree.outer_nodes,
    new OuterNode(
      { key: $.copy(key), value: value, parent_index: $.copy(inner_index) },
      new SimpleStructTag(OuterNode, [$p[0]])
    ),
    $c,
    [new SimpleStructTag(OuterNode, [$p[0]])]
  ...
  Stdlib.Vector.push_back_(
    ...
    new InnerNode(
      {
        critical_bit: $.copy(critical_bit),
        parent_index: $.copy(parent_index),
        left_child_index: $.copy(left_child_index),
        right_child_index: $.copy(right_child_index)
      },
      new SimpleStructTag(InnerNode)
    ),
    $c,
...
export function search_outer_(
  ...
  while (true) {
    ...
    if (is_outer_node_($.copy(index), $c)) {
      node = Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(index), $c), $c, [
        new SimpleStructTag(OuterNode, [$p[0]])
      ]);
...
export function singleton_(key: U128, value: any, $c: AptosDataCache, $p: TypeTag[] /* <V>*/): CritBitTree {
  let tree;
...
export function stitch_parent_of_child_(
  ...
  if (is_outer_node_($.copy(child_index), $c)) {
    Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(child_index), $c), $c, [
      new SimpleStructTag(OuterNode, [$p[0]])
    ]).parent_index = $.copy(new_index);
  } else {
    Stdlib.Vector.borrow_mut_(tree.inner_nodes, $.copy(child_index), $c, [
      new SimpleStructTag(InnerNode)
    ]).parent_index = $.copy(new_index);
  }
  return;
...
export function stitch_swap_remove_(
  ...
  if (is_outer_node_($.copy(node_index), $c)) {
    parent_index = $.copy(
      Stdlib.Vector.borrow_(tree.outer_nodes, outer_node_vector_index_($.copy(node_index), $c), $c, [
        new SimpleStructTag(OuterNode, [$p[0]])
      ]).parent_index
    );
    ...
    stitch_child_of_parent_(
      tree,
      $.copy(node_index),
      $.copy(parent_index),
      outer_node_child_index_($.copy(n_nodes).sub(u64('1')), $c),
      $c,
      [$p[0]]
    );
  } else {
    node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(node_index), $c, [new SimpleStructTag(InnerNode)]);
    [parent_index__1, left_child_index, right_child_index] = [
      $.copy(node.parent_index),
    ...
    stitch_child_of_parent_(tree, $.copy(node_index), $.copy(parent_index__1), $.copy(n_nodes).sub(u64('1')), $c, [
      $p[0]
    ]);
  }
  return;
...
export function traverse_end_pop_(
  ...
  if ($.copy(n_outer_nodes).eq(u64('1'))) {
    ...
    node_child_side = $.copy(
      Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent_index), $c, [new SimpleStructTag(InnerNode)])
        .left_child_index
    ).eq($.copy(child_index));
...
export function traverse_init_mut_(
  ...
  node = Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(child_field_index), $c), $c, [
    new SimpleStructTag(OuterNode, [$p[0]])
  ]);
...
export function traverse_mut_(
  ...
  node = Stdlib.Vector.borrow_mut_(tree.outer_nodes, outer_node_vector_index_($.copy(target_child_index), $c), $c, [
    new SimpleStructTag(OuterNode, [$p[0]])
  ]);
...
export function traverse_pop_mut_(
  ...
  start_child_side = $.copy(
    Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent_index), $c, [new SimpleStructTag(InnerNode)]).left_child_index
  ).eq($.copy(child_index));
  [temp$1, temp$2, temp$3, temp$4] = [tree, $.copy(key), $.copy(parent_index), direction];
  target_child_index = traverse_target_child_index_(temp$1, temp$2, temp$3, temp$4, $c, [$p[0]]);
  ...
  target_node = Stdlib.Vector.borrow_mut_(
    tree.outer_nodes,
    outer_node_vector_index_($.copy(target_child_index), $c),
    $c,
    [new SimpleStructTag(OuterNode, [$p[0]])]
  );
  return [
    $.copy(target_node.key),
    target_node.value,
    $.copy(target_node.parent_index),
...
export function traverse_target_child_index_(
  ...
  while (direction != is_set_($.copy(key), $.copy(parent.critical_bit), $c)) {
    {
      parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent.parent_index), $c, [
        new SimpleStructTag(InnerNode)
      ]);
    }
  }
  ...
  while (!is_outer_node_($.copy(child_index), $c)) {
    {
      if (direction == $.copy(LEFT)) {
        temp$2 = $.copy(
          Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_index), $c, [new SimpleStructTag(InnerNode)])
            .right_child_index
        );
      } else {
        temp$2 = $.copy(
...
export function loadParsers(repo: AptosParserRepo) {
  repo.addParser(
    '0xb1d4c0de8bc24468608637dfdbff975a0888f8935aa63338a44078eec5c7b6c7::critbit::CritBitTree',
    CritBitTree.CritBitTreeParser
  );
  repo.addParser(
    '0xb1d4c0de8bc24468608637dfdbff975a0888f8935aa63338a44078eec5c7b6c7::critbit::InnerNode',
    InnerNode.InnerNodeParser
  );
  repo.addParser(
    '0xb1d4c0de8bc24468608637dfdbff975a0888f8935aa63338a44078eec5c7b6c7::critbit::OuterNode',
