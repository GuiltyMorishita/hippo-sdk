...
export function borrow_(
  ...
  closest_outer_node_ref = borrow_closest_outer_node_(tree, $.copy(key), $c, [
    $p[0],
  ]);
...
export function borrow_closest_outer_node_(
  ...
  if (is_outer_node_($.copy(tree.root), $c)) {
    return Stdlib.Vector.borrow_(
      tree.outer_nodes,
      outer_node_vector_index_($.copy(tree.root), $c),
      $c,
      [new SimpleStructTag(OuterNode, [$p[0]])]
    );
  } else {
  }
  node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(tree.root), $c, [
  ...
  while (true) {
    ...
    if (is_outer_node_($.copy(child_index), $c)) {
      return Stdlib.Vector.borrow_(
        tree.outer_nodes,
        outer_node_vector_index_($.copy(child_index), $c),
        $c,
        [new SimpleStructTag(OuterNode, [$p[0]])]
      );
    } else {
    }
    node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_index), $c, [
      new SimpleStructTag(InnerNode),
    ]);
  }
}
...
export function borrow_closest_outer_node_mut_(
  ...
  let temp$1, child_index, node;
  if (is_outer_node_($.copy(tree.root), $c)) {
    return Stdlib.Vector.borrow_mut_(
      tree.outer_nodes,
      outer_node_vector_index_($.copy(tree.root), $c),
      $c,
      [new SimpleStructTag(OuterNode, [$p[0]])]
...
export function borrow_mut_(
  ...
  closest_outer_node_ref_mut = borrow_closest_outer_node_mut_(
    tree,
    $.copy(key),
    $c,
    [$p[0]]
...
export function check_length_(length: U64, $c: AptosDataCache): void {
  if (
    !$.copy(length).lt($.copy(HI_64).xor($.copy(OUTER).shl($.copy(NODE_TYPE))))
  ) {
...
export function destroy_empty_(
  ...
  Stdlib.Vector.destroy_empty_(inner_nodes, $c, [
    new SimpleStructTag(InnerNode),
  ]);
  Stdlib.Vector.destroy_empty_(outer_nodes, $c, [
    new SimpleStructTag(OuterNode, [$p[0]]),
  ]);
  return;
}
  ...
  return new CritBitTree(
    {
      ...
      outer_nodes: Stdlib.Vector.empty_($c, [
        new SimpleStructTag(OuterNode, [$p[0]]),
      ]),
    },
...
export function has_key_(
  ...
  return $.copy(
    borrow_closest_outer_node_(tree, $.copy(key), $c, [$p[0]]).key
  ).eq($.copy(key));
}
...
export function insert_above_(
  ...
  let node, node_index;
  node_index = $.copy(
    Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(search_parent_index), $c, [
      new SimpleStructTag(InnerNode),
    ]).parent_index
  );
  while (true) {
    if ($.copy(node_index).eq($.copy(ROOT))) {
      ...
      node = Stdlib.Vector.borrow_mut_(
        tree.inner_nodes,
        $.copy(node_index),
        $c,
        [new SimpleStructTag(InnerNode)]
...
export function insert_below_(
  ...
  let search_parent;
  search_parent = Stdlib.Vector.borrow_mut_(
    tree.inner_nodes,
  ...
  Stdlib.Vector.borrow_mut_(
    tree.outer_nodes,
    outer_node_vector_index_($.copy(search_index), $c),
    $c,
    [new SimpleStructTag(OuterNode, [$p[0]])]
  ).parent_index = $.copy(n_inner_nodes);
  push_back_insert_nodes_(
...
export function insert_below_walk_(
  ...
  review_node = Stdlib.Vector.borrow_mut_(
    tree.inner_nodes,
...
export function insert_general_(
  ...
  let temp$1,
    ...
    search_parent_index;
  n_inner_nodes = Stdlib.Vector.length_(tree.inner_nodes, $c, [
    new SimpleStructTag(InnerNode),
  ]);
  ...
  [
    ...
    search_parent_index,
    search_parent_critical_bit,
  ] = search_outer_(temp$1, temp$2, $c, [$p[0]]);
...
export function insert_singleton_(
  ...
  outer_node = Stdlib.Vector.borrow_(tree.outer_nodes, u64("0"), $c, [
    new SimpleStructTag(OuterNode, [$p[0]]),
  ]);
  ...
  tree.root = u64("0");
  Stdlib.Vector.borrow_mut_(tree.outer_nodes, u64("0"), $c, [
    new SimpleStructTag(OuterNode, [$p[0]]),
  ]).parent_index = u64("0");
  return;
}
...
export function is_empty_(
  ...
  return Stdlib.Vector.is_empty_(tree.outer_nodes, $c, [
    new SimpleStructTag(OuterNode, [$p[0]]),
  ]);
}
...
export function length_(
  ...
  return Stdlib.Vector.length_(tree.outer_nodes, $c, [
    new SimpleStructTag(OuterNode, [$p[0]]),
  ]);
}
...
export function max_node_child_index_(
  ...
  while (true) {
    ...
    child_field_index = $.copy(
      Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_field_index), $c, [
        new SimpleStructTag(InnerNode),
      ]).right_child_index
    );
  }
...
export function pop_destroy_nodes_(
  ...
  Stdlib.Vector.swap_remove_(tree.inner_nodes, $.copy(inner_index), $c, [
    new SimpleStructTag(InnerNode),
  ]);
  if ($.copy(inner_index).lt($.copy(n_inner_nodes).sub(u64("1")))) {
    stitch_swap_remove_(tree, $.copy(inner_index), $.copy(n_inner_nodes), $c, [
      $p[0],
    ]);
  } else {
  }
  ...
  if (
    outer_node_vector_index_($.copy(outer_index), $c).lt(
      $.copy(n_outer_nodes).sub(u64("1"))
    )
  ) {
    stitch_swap_remove_(tree, $.copy(outer_index), $.copy(n_outer_nodes), $c, [
      $p[0],
    ]);
  } else {
  }
...
export function pop_general_(
  ...
  let temp$1,
    ...
    search_parent_index;
  [temp$1, temp$2] = [tree, $.copy(key)];
  [search_index, search_child_side, search_key, search_parent_index] =
    search_outer_(temp$1, temp$2, $c, [$p[0]]);
  if (!$.copy(search_key).eq($.copy(key))) {
    throw $.abortCode($.copy(E_NOT_HAS_KEY));
  }
  pop_update_relationships_(
    tree,
    search_child_side,
    $.copy(search_parent_index),
    $c,
...
export function pop_singleton_(
  ...
  if (
    !$.copy(
      Stdlib.Vector.borrow_(tree.outer_nodes, u64("0"), $c, [
        new SimpleStructTag(OuterNode, [$p[0]]),
      ]).key
    ).eq($.copy(key))
  ) {
  ...
  tree.root = u64("0");
  let { value: value } = Stdlib.Vector.pop_back_(tree.outer_nodes, $c, [
    new SimpleStructTag(OuterNode, [$p[0]]),
  ]);
  return value;
}
...
export function pop_update_relationships_(
  ...
  parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent_index), $c, [
    new SimpleStructTag(InnerNode),
  ]);
  ...
  if (is_outer_node_($.copy(sibling_index), $c)) {
    Stdlib.Vector.borrow_mut_(
      tree.outer_nodes,
      outer_node_vector_index_($.copy(sibling_index), $c),
      $c,
      [new SimpleStructTag(OuterNode, [$p[0]])]
  ...
  if ($.copy(grandparent_index).eq($.copy(ROOT))) {
    ...
    grandparent = Stdlib.Vector.borrow_mut_(
      tree.inner_nodes,
      $.copy(grandparent_index),
      $c,
      [new SimpleStructTag(InnerNode)]
    );
...
export function search_outer_(
  ...
  parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(tree.root), $c, [
    new SimpleStructTag(InnerNode),
  ]);
  while (true) {
    ...
    if (is_outer_node_($.copy(index), $c)) {
      node = Stdlib.Vector.borrow_(
        tree.outer_nodes,
        outer_node_vector_index_($.copy(index), $c),
        $c,
        [new SimpleStructTag(OuterNode, [$p[0]])]
      );
      return [
        ...
        $.copy(parent.critical_bit),
      ];
    } else {
    }
    parent = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(index), $c, [
      new SimpleStructTag(InnerNode),
    ]);
  }
}
...
export function stitch_child_of_parent_(
  ...
  let parent;
  parent = Stdlib.Vector.borrow_mut_(
    tree.inner_nodes,
    $.copy(parent_index),
    $c,
    [new SimpleStructTag(InnerNode)]
...
export function stitch_swap_remove_(
  ...
  if (is_outer_node_($.copy(node_index), $c)) {
    parent_index = $.copy(
      Stdlib.Vector.borrow_(
        tree.outer_nodes,
        outer_node_vector_index_($.copy(node_index), $c),
        $c,
        [new SimpleStructTag(OuterNode, [$p[0]])]
      ).parent_index
    );
    ...
    node = Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(node_index), $c, [
      new SimpleStructTag(InnerNode),
    ]);
    ...
    stitch_parent_of_child_(
      tree,
      $.copy(node_index),
      $.copy(left_child_index),
      $c,
      [$p[0]]
    );
    stitch_parent_of_child_(
    ...
    stitch_child_of_parent_(
      tree,
      $.copy(node_index),
      $.copy(parent_index__1),
      $.copy(n_nodes).sub(u64("1")),
      $c,
      [$p[0]]
    );
  }
...
export function traverse_end_pop_(
  ...
  if ($.copy(n_outer_nodes).eq(u64("1"))) {
    ...
    node_child_side = $.copy(
      Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(parent_index), $c, [
        new SimpleStructTag(InnerNode),
      ]).left_child_index
    ).eq($.copy(child_index));
    pop_update_relationships_(tree, node_child_side, $.copy(parent_index), $c, [
      $p[0],
    ]);
    temp$1 = pop_destroy_nodes_(
      tree,
      $.copy(parent_index),
      $.copy(child_index),
      $.copy(n_outer_nodes),
...
export function traverse_init_mut_(
  ...
  node = Stdlib.Vector.borrow_mut_(
    tree.outer_nodes,
    outer_node_vector_index_($.copy(child_field_index), $c),
    $c,
    [new SimpleStructTag(OuterNode, [$p[0]])]
  );
  return [
...
export function traverse_mut_(
  ...
  target_child_index = traverse_target_child_index_(
    temp$1,
    temp$2,
    temp$3,
    temp$4,
...
export function traverse_pop_mut_(
  ...
  pop_update_relationships_(tree, start_child_side, $.copy(parent_index), $c, [
    $p[0],
  ]);
  start_value = pop_destroy_nodes_(
    tree,
    $.copy(parent_index),
    $.copy(child_index),
    $.copy(n_outer_nodes),
  ...
  if (
    outer_node_vector_index_($.copy(target_child_index), $c).eq(
      $.copy(n_outer_nodes).sub(u64("1"))
    )
  ) {
...
export function traverse_target_child_index_(
  ...
  while (direction != is_set_($.copy(key), $.copy(parent.critical_bit), $c)) {
    {
      parent = Stdlib.Vector.borrow_(
        tree.inner_nodes,
        $.copy(parent.parent_index),
        $c,
        [new SimpleStructTag(InnerNode)]
      );
    }
  ...
  while (!is_outer_node_($.copy(child_index), $c)) {
    {
      if (direction == $.copy(LEFT)) {
        temp$2 = $.copy(
          Stdlib.Vector.borrow_(tree.inner_nodes, $.copy(child_index), $c, [
            new SimpleStructTag(InnerNode),
          ]).right_child_index
        );
      } else {
