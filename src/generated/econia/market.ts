...
export function book_orders_sdk_ (
  ...
  return [get_orders_sdk_(order_book_ref_mut, $.copy(ASK), $c, [$p[0], $p[1], $p[2]]), get_orders_sdk_(order_book_ref_mut, $.copy(BID), $c, [$p[0], $p[1], $p[2]])];
}
...
export function book_price_levels_sdk_ (
  ...
  return [get_price_levels_sdk_(get_orders_sdk_(order_book_ref_mut, $.copy(ASK), $c, [$p[0], $p[1], $p[2]]), $c), get_price_levels_sdk_(get_orders_sdk_(order_book_ref_mut, $.copy(BID), $c, [$p[0], $p[1], $p[2]]), $c)];
}
...
export function cancel_limit_order_ (
  ...
  order_book_ref_mut = $c.borrow_global_mut<OrderBook>(new SimpleStructTag(OrderBook, [$p[0], $p[1], $p[2]]), $.copy(host));
  if ((side == $.copy(ASK))) {
  ...
  let { user: order_user, custodian_id: order_custodian_id } = Critbit.pop_(tree_ref_mut, $.copy(order_id), $c, [new SimpleStructTag(Order)]);
...
export function fill_market_order_loop_order_follow_up_ (
  ...
  if (($.copy(n_orders)).eq((u64("1")))) {
    ...
    if (complete_fill) {
      [target_order_id, target_order_ref_mut, target_parent_index, target_child_index, {  }] = Critbit.traverse_pop_mut_(tree_ref_mut, $.copy(target_order_id), $.copy(target_parent_index), $.copy(target_child_index), $.copy(n_orders), traversal_direction, $c, [new SimpleStructTag(Order)]);
...
export function fill_market_order_traverse_loop_ (
  ...
  [target_order_id, target_order_ref_mut, target_parent_index, target_child_index] = Critbit.traverse_init_mut_(tree_ref_mut, traversal_direction, $c, [new SimpleStructTag(Order)]);
...
export function get_econia_capability_ (
  ...
  return $.copy($c.borrow_global<EconiaCapabilityStore>(new SimpleStructTag(EconiaCapabilityStore), new HexString("0xb1d4c0de8bc24468608637dfdbff975a0888f8935aa63338a44078eec5c7b6c7")).econia_capability);
}
...
export function init_econia_capability_store_ (
  ...
  $c.move_to(new SimpleStructTag(EconiaCapabilityStore), account, new EconiaCapabilityStore({ econia_capability: $.copy(econia_capability) }, new SimpleStructTag(EconiaCapabilityStore)));
  return;
}
...
export function place_limit_order_ (
  ...
  Critbit.insert_(tree_ref_mut, $.copy(order_id), new Order({ base_parcels: $.copy(base_parcels), user: $.copy(user), custodian_id: $.copy(custodian_id) }, new SimpleStructTag(Order)), $c, [new SimpleStructTag(Order)]);
  return;
}
...
export class App {
  ...
  get EconiaCapabilityStore() { return EconiaCapabilityStore; }
