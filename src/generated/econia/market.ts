...
export const MIN_ASK_DEFAULT: U128 = u128(
...
export class EconiaCapabilityStore {
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    this.econia_capability = proto[
      "econia_capability"
    ] as Capability.EconiaCapability;
  }

  static EconiaCapabilityStoreParser(
    data: any,
    typeTag: TypeTag,
...
export function book_price_levels_sdk_(
  ...
  return [
    get_price_levels_sdk_(
      get_orders_sdk_(order_book_ref_mut, $.copy(ASK), $c, [
        $p[0],
        $p[1],
        $p[2],
      ]),
    ...
    get_price_levels_sdk_(
      get_orders_sdk_(order_book_ref_mut, $.copy(BID), $c, [
        $p[0],
        $p[1],
        $p[2],
      ]),
...
export function cancel_limit_order_(
  ...
  let { user: order_user, custodian_id: order_custodian_id } = Critbit.pop_(
    tree_ref_mut,
    $.copy(order_id),
    $c,
  ...
  if (temp$4) {
    ...
    if (temp$6) {
      if (Critbit.is_empty_(tree_ref_mut, $c, [new SimpleStructTag(Order)])) {
        ...
        temp$7 = Critbit.max_key_(tree_ref_mut, $c, [
          new SimpleStructTag(Order),
        ]);
      }
  ...
  User.remove_order_internal_(temp$12, temp$11, temp$10, temp$9, temp$8, $c, [
    $p[0],
    $p[1],
    $p[2],
  ]);
...
export function fill_market_order_break_cleanup_(
  ...
  if (should_pop) {
    Critbit.pop_(tree_ref_mut, $.copy(target_order_id), $c, [
      new SimpleStructTag(Order),
    ]);
  } else {
  }
...
export function fill_market_order_check_base_parcels_to_fill_(
  ...
  base_parcels_can_afford = Stdlib.Coin.value_(quote_coins_ref, $c, [
    $p[0],
  ]).div($.copy(target_price));
  if (
    $.copy(base_parcels_can_afford).lt(
      $.copy(target_order_ref_mut.base_parcels)
    )
  ) {
    if (
      $.copy(base_parcels_can_afford).lt($.copy(base_parcels_to_fill_ref_mut))
    ) {
...
export function fill_market_order_from_market_account_(
  ...
  market_account_info = User.market_account_info_($.copy(custodian_id), $c, [
    $p[0],
    $p[1],
    $p[2],
  ]);
...
export function fill_market_order_init_(
  ...
  [side, tree_ref_mut, spread_maker_ref_mut, traversal_direction] = [
    temp$1,
    temp$2,
    temp$3,
    temp$4,
...
export function fill_market_order_loop_order_follow_up_(
  ...
  [new_spread_maker, should_break, should_pop] = [
...
export function fill_market_order_process_loop_order_(
  ...
  fill_market_order_check_base_parcels_to_fill_(
    temp$1,
    temp$2,
    temp$3,
    temp$4,
  ...
  complete_fill = $.copy(base_parcels_to_fill_ref_mut).ge(
    $.copy(target_order_ref_mut.base_parcels)
  );
  if (complete_fill) {
  ...
  $.set(
    base_parcels_to_fill_ref_mut,
    $.copy(base_parcels_to_fill_ref_mut).sub($.copy(base_parcels_filled))
  );
  ...
  if (!complete_fill) {
    target_order_ref_mut.base_parcels = $.copy(
      target_order_ref_mut.base_parcels
    ).sub($.copy(base_parcels_filled));
  } else {
  }
...
export function fill_market_order_traverse_loop_(
  ...
  [
    ...
    target_parent_index,
    target_child_index,
  ] = Critbit.traverse_init_mut_(tree_ref_mut, traversal_direction, $c, [
    new SimpleStructTag(Order),
  ]);
  ...
  null_order = new Order(
    { base_parcels: temp$3, user: temp$1, custodian_id: temp$2 },
    new SimpleStructTag(Order)
  );
  while (true) {
...
export function get_orders_sdk_(
  ...
  remaining_traversals = Critbit.length_(tree_ref_mut, $c, [
    new SimpleStructTag(Order),
  ]).sub(u64("1"));
  [target_id, target_order_ref_mut, target_parent_index] =
    Critbit.traverse_init_mut_(tree_ref_mut, traversal_direction, $c, [
      new SimpleStructTag(Order),
    ]);
  while (true) {
    Stdlib.Vector.push_back_(
      ...
      new SimpleOrder(
        {
          price: Order_id.price_($.copy(target_id), $c),
          base_parcels: $.copy(target_order_ref_mut.base_parcels),
        },
...
export function get_price_levels_sdk_(
  ...
  if (
    Stdlib.Vector.is_empty_(simple_orders, $c, [
      new SimpleStructTag(SimpleOrder),
    ])
  ) {
  ...
  simple_order_ref = Stdlib.Vector.borrow_(simple_orders, u64("0"), $c, [
    new SimpleStructTag(SimpleOrder),
  ]);
  level_price = $.copy(simple_order_ref.price);
  level_base_parcels = $.copy(simple_order_ref.base_parcels);
  n_simple_orders = Stdlib.Vector.length_(simple_orders, $c, [
    new SimpleStructTag(SimpleOrder),
  ]);
  simple_order_index = u64("1");
  while ($.copy(simple_order_index).lt($.copy(n_simple_orders))) {
    {
      simple_order_ref = Stdlib.Vector.borrow_(
        simple_orders,
        $.copy(simple_order_index),
        $c,
        [new SimpleStructTag(SimpleOrder)]
      );
      if ($.copy(simple_order_ref.price).neq($.copy(level_price))) {
        Stdlib.Vector.push_back_(
          ...
          new PriceLevel(
            {
              price: $.copy(level_price),
              base_parcels: $.copy(level_base_parcels),
            },
        ...
        [level_price, level_base_parcels] = [
          $.copy(simple_order_ref.price),
          $.copy(simple_order_ref.base_parcels),
        ];
      } else {
        level_base_parcels = $.copy(level_base_parcels).add(
          $.copy(simple_order_ref.base_parcels)
        );
      }
...
export function place_limit_order_(
  ...
  order_id = Order_id.order_id_(
    $.copy(price),
    get_serial_id_(order_book_ref_mut, $c, [$p[0], $p[1], $p[2]]),
    side,
    $c
  );
  temp$7 = $.copy(user);
  ...
  [tree_ref_mut, new_spread_maker, crossed_spread, spread_maker_ref_mut] = [
    temp$8,
    temp$9,
    temp$10,
    temp$11,
  ...
  Critbit.insert_(
    ...
    new Order(
      {
        base_parcels: $.copy(base_parcels),
        user: $.copy(user),
...
export function register_market_(
  ...
  init_book_(host, Registry.scale_factor_($c, [$p[2]]), $c, [
    $p[0],
    $p[1],
    $p[2],
  ]);
...
export function simulate_swap_sdk_(
  ...
  side = temp$1;
  simple_orders = get_orders_sdk_(order_book_ref_mut, side, $c, [
    $p[0],
    $p[1],
    $p[2],
  ]);
  ...
  [coins_in_left, coins_out, simple_order_index, n_orders] = [
    ...
    u64("0"),
    u64("0"),
    Stdlib.Vector.length_(simple_orders, $c, [
      new SimpleStructTag(SimpleOrder),
    ]),
  ];
  while (true) {
    simple_order = Stdlib.Vector.borrow_(
      simple_orders,
      $.copy(simple_order_index),
      $c,
      [new SimpleStructTag(SimpleOrder)]
    ...
    if (style == $.copy(SELL)) {
      ...
      base_parcels_can_afford = $.copy(coins_in_left).div(
        $.copy(simple_order.price)
      );
      if (
        $.copy(simple_order.base_parcels).gt($.copy(base_parcels_can_afford))
      ) {
    ...
    coins_in_left = $.copy(coins_in_left).sub(
      $.copy(base_parcels_filled).mul($.copy(coins_in_multiplier))
    );
    coins_out = $.copy(coins_out).add(
      $.copy(base_parcels_filled).mul($.copy(coins_out_multiplier))
    );
    simple_order_index = $.copy(simple_order_index).add(u64("1"));
    if (should_return) {
      temp$8 = true;
...
export function swap_(
  ...
  if (style == $.copy(BUY)) {
    [temp$1, temp$2] = [
      $.copy(HI_64),
      Stdlib.Coin.value_(quote_coins_ref_mut, $c, [$p[1]]),
    ];
  } else {
    [temp$1, temp$2] = [
      Stdlib.Coin.value_(base_coins_ref_mut, $c, [$p[0]]).div(
        $.copy(scale_factor)
      ),
      u64("0"),
    ];
...
export class App {
  ...
  async loadEconiaCapabilityStore(owner: HexString, loadFull = true) {
    const val = await EconiaCapabilityStore.load(
      this.repo,
      this.client,
      owner,
      [] as TypeTag[]
  ...
  payload_cancel_limit_order_user(
    ...
    | TxnBuilderTypes.TransactionPayloadEntryFunction
    | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_cancel_limit_order_user(
      host,
      side,
      order_id,
      $p,
  ...
  async cancel_limit_order_user(
    ...
    const payload = buildPayload_cancel_limit_order_user(
      host,
      side,
      order_id,
      $p,
  ...
  payload_fill_market_order_user(
    ...
    | TxnBuilderTypes.TransactionPayloadEntryFunction
    | Types.TransactionPayload_EntryFunctionPayload {
    return buildPayload_fill_market_order_user(
      host,
      style,
      max_base_parcels,
      max_quote_units,
  ...
  async fill_market_order_user(
    ...
    const payload = buildPayload_fill_market_order_user(
      host,
      style,
      max_base_parcels,
      max_quote_units,
