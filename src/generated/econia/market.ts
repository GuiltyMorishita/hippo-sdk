...
export function cancel_all_limit_orders_(
  ...
  while ($.copy(n_orders).gt(u64('0'))) {
    {
      order_id_nearest_spread = User.get_order_id_nearest_spread_internal_(
        $.copy(user),
      ...
      cancel_limit_order_(
        ...
        $.copy(host),
        $.copy(market_id),
        $.copy(general_custodian_id),
        side,
        $.copy(order_id_nearest_spread),
        $c
      );
...
export function cancel_all_limit_orders_custodian_(
  ...
  cancel_all_limit_orders_(
    $.copy(user),
    $.copy(host),
    $.copy(market_id),
    Registry.custodian_id_(general_custodian_capability_ref, $c),
    side,
    $c
  );
  return;
...
export function cancel_limit_order_(
  ...
  order_book_ref_mut = Open_table.borrow_mut_(order_books_map_ref_mut, $.copy(market_id), $c, [
    AtomicTypeTag.U64,
  ...
  let { user: order_user, general_custodian_id: order_general_custodian_id } = Critbit.pop_(
    tree_ref_mut,
    $.copy(order_id),
    $c,
  ...
  [market_account_id, lot_size, tick_size] = [
    User.get_market_account_id_($.copy(market_id), $.copy(general_custodian_id), $c),
    $.copy(order_book_ref_mut.lot_size),
    $.copy(order_book_ref_mut.tick_size)
  ];
  User.remove_order_internal_(
    $.copy(user),
    $.copy(market_account_id),
    $.copy(lot_size),
    $.copy(tick_size),
...
export function buildPayload_cancel_limit_order_user(
  ...
  return $.buildPayload(
    ...
    typeParamStrings,
    [host, market_id, side, order_id],
    isJSON
  );
}

export function get_counter_(order_book_ref_mut: OrderBook, $c: AptosDataCache): U64 {
...
export function match_(
  ...
  [lots_until_max, ticks_until_max, side, tree_ref_mut, spread_maker_ref_mut, n_orders, traversal_direction] =
    match_init_(order_book_ref_mut, direction_ref, max_lots_ref, max_ticks_ref, $c, [$p[0], $p[1]]);
...
export function match_from_market_account_(
  ...
  [, base_available, base_ceiling, , quote_available, quote_ceiling] = User.get_asset_counts_internal_(
    $.copy(user_ref),
    $.copy(market_account_id_ref),
    $c
  );
  match_range_check_fills_(
    direction_ref,
    min_base_ref,
    max_base_ref,
    min_quote_ref,
  ...
  [optional_base_coins, optional_quote_coins] = User.withdraw_assets_as_option_internal_(
    $.copy(user_ref),
    $.copy(market_account_id_ref),
    $.copy(base_to_withdraw),
    $.copy(quote_to_withdraw),
    $.copy(order_book_ref_mut.generic_asset_transfer_custodian_id),
    $c,
    [$p[0], $p[1]]
  );
  ticks_filled = u64('0');
  ...
  if ($.copy(direction_ref) == $.copy(BUY)) {
    [temp$15, temp$16] = [
      $.copy(lots_filled_ref_mut).mul($.copy(lot_size)),
      $.copy(max_quote_ref).sub($.copy(ticks_filled).mul($.copy(tick_size)))
    ];
  } else {
    [temp$15, temp$16] = [
      $.copy(max_base_ref).sub($.copy(lots_filled_ref_mut).mul($.copy(lot_size))),
      $.copy(ticks_filled).mul($.copy(tick_size))
    ];
  }
  [base_on_hand, quote_on_hand] = [temp$15, temp$16];
  User.deposit_assets_as_option_internal_(
    $.copy(user_ref),
    $.copy(market_account_id_ref),
    $.copy(base_on_hand),
    $.copy(quote_on_hand),
...
export function match_loop_(
  ...
  [
    ...
    should_pop_last,
    new_spread_maker
  ] = match_loop_init_(tree_ref_mut, traversal_direction_ref, $c);
  while (true) {
    match_loop_order_(
      market_id_ref,
      side_ref,
      lot_size_ref,
      tick_size_ref,
      lots_until_max_ref_mut,
      ticks_until_max_ref_mut,
      limit_price_ref,
      target_order_id,
      target_order_ref_mut,
      complete_target_fill,
    ...
    [target_order_id, target_order_ref_mut, should_break] = match_loop_order_follow_up_(
      tree_ref_mut,
      side_ref,
...
export function match_loop_order_(
  ...
  let temp$1,
    ...
    fill_size,
    quote_to_route,
    target_order_market_account_id,
    target_order_price,
    ticks_filled;
  ...
  [base_to_route, quote_to_route] = [
    $.copy(fill_size).mul($.copy(lot_size_ref)),
    $.copy(ticks_filled).mul($.copy(tick_size_ref))
  ];
  target_order_market_account_id = User.get_market_account_id_(
    $.copy(market_id_ref),
    $.copy(target_order_ref_mut.general_custodian_id),
    $c
  );
  User.fill_order_internal_(
    $.copy(target_order_ref_mut.user),
    $.copy(target_order_market_account_id),
    $.copy(side_ref),
    $.copy(target_order_id_ref),
    $.copy(complete_target_fill_ref_mut),
    $.copy(fill_size),
    optional_base_coins_ref_mut,
    optional_quote_coins_ref_mut,
    $.copy(base_to_route),
...
export function match_loop_order_follow_up_(
  ...
  if ($.copy(n_orders_ref_mut).eq(u64('1'))) {
    ...
    if ($.copy(complete_target_fill_ref)) {
      [target_order_id, target_order_ref_mut, target_parent_index, target_child_index, empty_order] =
        Critbit.traverse_pop_mut_(
          tree_ref_mut,
          $.copy(target_order_id),
          $.copy(target_parent_index_ref_mut),
          $.copy(target_child_index_ref_mut),
          $.copy(n_orders_ref_mut),
          $.copy(traversal_direction_ref),
          $c,
          [new SimpleStructTag(Order)]
        );
...
export function match_range_check_fills_(
  ...
  if ($.copy(direction_ref) == $.copy(BUY)) {
    [temp$1, temp$2, temp$3, temp$4] = [
      $.copy(base_ceiling_ref),
      $.copy(max_base_ref),
      $.copy(quote_available_ref),
      $.copy(max_quote_ref)
    ];
  } else {
    [temp$1, temp$2, temp$3, temp$4] = [
      $.copy(quote_ceiling_ref),
      $.copy(max_quote_ref),
      $.copy(base_available_ref),
      $.copy(max_base_ref)
    ];
  }
...
export function orders_vector_(order_book_ref_mut: OrderBook, side: boolean, $c: AptosDataCache): SimpleOrder[] {
  let temp$1,
    temp$2,
  ...
  while (true) {
    Stdlib.Vector.push_back_(
      simple_orders,
      new SimpleOrder(
        { price: Order_id.price_($.copy(target_id), $c), size: $.copy(target_order_ref_mut.size) },
...
export function orders_vectors_(order_book_ref_mut: OrderBook, $c: AptosDataCache): [SimpleOrder[], SimpleOrder[]] {
...
export function place_limit_order_(
  ...
  order_book_ref_mut = Open_table.borrow_mut_(order_books_map_ref_mut, $.copy(market_id_ref), $c, [
    AtomicTypeTag.U64,
    new SimpleStructTag(OrderBook)
  ]);
  [market_account_id, lot_size, tick_size, direction, min_base, max_base, max_quote, lots_filled] = [
    u128('0'),
    u64('0'),
    u64('0'),
    false,
...
export function place_limit_order_post_match_(
  ...
  Critbit.insert_(
    ...
    $.copy(order_id),
    new Order(
      { size: $.copy(size_to_fill), user: $.copy(user_ref), general_custodian_id: $.copy(general_custodian_id_ref) },
      new SimpleStructTag(Order)
    ),
    $c,
    [new SimpleStructTag(Order)]
...
export function place_limit_order_pre_match_(
  ...
  $.set(
    market_account_id_ref_mut,
    User.get_market_account_id_($.copy(market_id_ref), $.copy(general_custodian_id_ref), $c)
  );
  if ($.copy(side_ref) == $.copy(ASK)) {
    temp$5 = $.copy(SELL);
  } else {
  ...
  if (temp$7) {
    [, , , , , quote_ceiling] = User.get_asset_counts_internal_(
      $.copy(user_ref),
      $.copy(market_account_id_ref_mut),
      $c
    );
...
export function buildPayload_place_limit_order_user(
  ...
  return $.buildPayload(
    ...
    typeParamStrings,
    [host, market_id, side, size, price, post_or_abort, fill_or_abort, immediate_or_cancel],
    isJSON
  );
}
...
export function buildPayload_place_market_order_user(
  ...
  return $.buildPayload(
    ...
    typeParamStrings,
    [host, market_id, direction, min_base, max_base, min_quote, max_quote, limit_price],
    isJSON
  );
}

export function price_levels_vector_(simple_orders: SimpleOrder[], $c: AptosDataCache): PriceLevel[] {
  ...
  while ($.copy(simple_order_index).lt($.copy(n_simple_orders))) {
    {
      ...
      if ($.copy(simple_order_ref.price).neq($.copy(level_price))) {
        Stdlib.Vector.push_back_(
          price_levels,
          new PriceLevel({ price: $.copy(level_price), size: $.copy(level_size) }, new SimpleStructTag(PriceLevel)),
          $c,
...
export function price_levels_vectors_(order_book_ref_mut: OrderBook, $c: AptosDataCache): [PriceLevel[], PriceLevel[]] {
  return [
    price_levels_vector_(orders_vector_(order_book_ref_mut, $.copy(ASK), $c), $c),
    price_levels_vector_(orders_vector_(order_book_ref_mut, $.copy(BID), $c), $c)
  ];
}
...
export function register_market_(
  ...
  let market_id;
  market_id = Registry.register_market_internal_(
    Stdlib.Signer.address_of_(host, $c),
    $.copy(lot_size),
    $.copy(tick_size),
...
export function register_market_generic_(
  ...
  register_market_(
    ...
    $.copy(lot_size),
    $.copy(tick_size),
    Registry.custodian_id_(generic_asset_transfer_custodian_id_ref, $c),
    $c,
    [$p[0], $p[1]]
  );
  return;
...
export function register_order_book_(
  ...
  if (!$c.exists(new SimpleStructTag(OrderBooks), $.copy(host_address))) {
    $c.move_to(
      ...
      host,
      new OrderBooks(
        { map: Open_table.empty_($c, [AtomicTypeTag.U64, new SimpleStructTag(OrderBook)]) },
        new SimpleStructTag(OrderBooks)
      )
    );
  } else {
    ...
    new OrderBook(
      {
        ...
        generic_asset_transfer_custodian_id: $.copy(generic_asset_transfer_custodian_id),
...
export function swap_(
  ...
  if (
    !$.copy(generic_asset_transfer_custodian_id_ref).eq($.copy(order_book_ref_mut.generic_asset_transfer_custodian_id))
  ) {
    throw $.abortCode($.copy(E_INVALID_CUSTODIAN));
  }
  lot_size = $.copy(order_book_ref_mut.lot_size);
...
export function swap_between_coinstores_(
  ...
  if (direction == $.copy(BUY)) {
    [temp$1, temp$2] = [
      ...
      Stdlib.Option.some_(Stdlib.Coin.withdraw_(user, $.copy(max_quote), $c, [$p[1]]), $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[1]])
      ])
    ];
  } else {
    [temp$1, temp$2] = [
      Stdlib.Option.some_(Stdlib.Coin.withdraw_(user, $.copy(max_base), $c, [$p[0]]), $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[0]])
      ]),
  ...
  Stdlib.Coin.deposit_(
    $.copy(user_address),
    Stdlib.Option.destroy_some_(optional_base_coins, $c, [
      new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[0]])
    ]),
    $c,
    [$p[0]]
  ...
  Stdlib.Coin.deposit_(
    $.copy(user_address),
    Stdlib.Option.destroy_some_(optional_quote_coins, $c, [
      new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[1]])
    ]),
    $c,
    [$p[1]]
...
export function swap_coins_(
  ...
  if (direction == $.copy(BUY)) {
    [temp$1, temp$2] = [
      ...
      Stdlib.Option.some_(Stdlib.Coin.extract_(quote_coins_ref_mut, $.copy(max_quote), $c, [$p[1]]), $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[1]])
      ])
    ];
  } else {
    [temp$1, temp$2] = [
      Stdlib.Option.some_(Stdlib.Coin.extract_(base_coins_ref_mut, $.copy(max_base), $c, [$p[0]]), $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[0]])
      ]),
      Stdlib.Option.some_(Stdlib.Coin.zero_($c, [$p[1]]), $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[1]])
...
export function swap_generic_(
  ...
  if (
    !(
      base_is_coin ==
      Stdlib.Option.is_some_(optional_base_coins_ref_mut, $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[0]])
      ])
    )
  ) {
  ...
  if (
    !(
      quote_is_coin ==
      Stdlib.Option.is_some_(optional_quote_coins_ref_mut, $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[1]])
      ])
    )
  ) {
  ...
  if (base_is_coin) {
    temp$2 = Stdlib.Coin.value_(
      Stdlib.Option.borrow_(optional_base_coins_ref_mut, $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[0]])
      ]),
      $c,
      [$p[0]]
  ...
  base_value = temp$2;
  if (quote_is_coin) {
    temp$4 = Stdlib.Coin.value_(
      Stdlib.Option.borrow_(optional_quote_coins_ref_mut, $c, [
        new StructTag(new HexString('0x1'), 'coin', 'Coin', [$p[1]])
      ]),
      $c,
      [$p[1]]
...
export function verify_order_book_exists_(host: HexString, market_id: U64, $c: AptosDataCache): void {
  ...
  if (
    !Open_table.contains_(order_books_map_ref, $.copy(market_id), $c, [
      AtomicTypeTag.U64,
      new SimpleStructTag(OrderBook)
    ])
  ) {
...
export function loadParsers(repo: AptosParserRepo) {
  repo.addParser(
    '0xe56148c106146758a4172a7189cd8487f84997de6f6c2b3396106a8f82cb0c33::market::Order',
    Order.OrderParser
  );
  repo.addParser(
    '0xe56148c106146758a4172a7189cd8487f84997de6f6c2b3396106a8f82cb0c33::market::OrderBook',
    OrderBook.OrderBookParser
  );
  repo.addParser(
    '0xe56148c106146758a4172a7189cd8487f84997de6f6c2b3396106a8f82cb0c33::market::OrderBooks',
    OrderBooks.OrderBooksParser
  );
  repo.addParser(
    '0xe56148c106146758a4172a7189cd8487f84997de6f6c2b3396106a8f82cb0c33::market::PriceLevel',
    PriceLevel.PriceLevelParser
  );
  repo.addParser(
    '0xe56148c106146758a4172a7189cd8487f84997de6f6c2b3396106a8f82cb0c33::market::SimpleOrder',
...
export class App {
  ...
  payload_place_limit_order_user(
    ...
    return buildPayload_place_limit_order_user(
      host,
      market_id,
      side,
      size,
  ...
  payload_place_market_order_user(
    ...
    return buildPayload_place_market_order_user(
      host,
      market_id,
      direction,
      min_base,
  ...
  payload_swap_between_coinstores(
    ...
    return buildPayload_swap_between_coinstores(
      host,
      market_id,
      direction,
      min_base,
