...
export function add_order_internal_ (
  ...
  if (!Open_table.contains_(temp$1, temp$2, $c, [new SimpleStructTag(MarketAccountInfo), new SimpleStructTag(MarketAccount)])) {
    throw $.abortCode($.copy(E_NO_MARKET_ACCOUNT));
  }
  market_account = Open_table.borrow_mut_(market_accounts_map, $.copy(market_account_info), $c, [new SimpleStructTag(MarketAccountInfo), new SimpleStructTag(MarketAccount)]);
...
export function exists_market_account_ (
  ...
  return Open_table.contains_(market_accounts_map, $.copy(market_account_info), $c, [new SimpleStructTag(MarketAccountInfo), new SimpleStructTag(MarketAccount)]);
}
...
export function fill_order_update_market_account_ (
  ...
  market_account_ref_mut = Open_table.borrow_mut_(market_accounts_map_ref_mut, $.copy(market_account_info), $c, [new SimpleStructTag(MarketAccountInfo), new SimpleStructTag(MarketAccount)]);
  if ((side == $.copy(ASK))) {
...
export function market_account_info_ (
  ...
  return new MarketAccountInfo({ market_info: Registry.market_info_($c, [$p[0], $p[1], $p[2]]), custodian_id: $.copy(custodian_id) }, new SimpleStructTag(MarketAccountInfo));
}
...
export function register_collateral_entry_ (
  ...
  map = $c.borrow_global_mut<Collateral>(new SimpleStructTag(Collateral, [$p[0]]), $.copy(user_address)).map;
...
export function register_market_account_ (
  ...
  market_account_info = new MarketAccountInfo({ market_info: $.copy(market_info), custodian_id: $.copy(custodian_id) }, new SimpleStructTag(MarketAccountInfo));
...
export function register_market_accounts_entry_ (
  ...
  map = $c.borrow_global_mut<MarketAccounts>(new SimpleStructTag(MarketAccounts), $.copy(user_address)).map;
  [temp$1, temp$2] = [map, $.copy(market_account_info)];
  if (!!Open_table.contains_(temp$1, temp$2, $c, [new SimpleStructTag(MarketAccountInfo), new SimpleStructTag(MarketAccount)])) {
    throw $.abortCode($.copy(E_MARKET_ACCOUNT_REGISTERED));
  }
  scale_factor = Registry.scale_factor_from_market_info_(market_account_info.market_info, $c);
  Open_table.add_(map, $.copy(market_account_info), new MarketAccount({ scale_factor: $.copy(scale_factor), asks: Critbit.empty_($c, [AtomicTypeTag.U64]), bids: Critbit.empty_($c, [AtomicTypeTag.U64]), base_coins_total: u64("0"), base_coins_available: u64("0"), quote_coins_total: u64("0"), quote_coins_available: u64("0") }, new SimpleStructTag(MarketAccount)), $c, [new SimpleStructTag(MarketAccountInfo), new SimpleStructTag(MarketAccount)]);
  return;
}
