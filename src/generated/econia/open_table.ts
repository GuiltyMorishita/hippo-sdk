...
export function add_(open_table: OpenTable, key: any, value: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): void {
  Stdlib.Table.add_(open_table.base_table, $.copy(key), value, $c, [$p[0], $p[1]]);
  Stdlib.Vector.push_back_(open_table.keys, $.copy(key), $c, [$p[0]]);
  return;
}

export function borrow_(open_table: OpenTable, key: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): any {
  return Stdlib.Table.borrow_(open_table.base_table, $.copy(key), $c, [$p[0], $p[1]]);
}

export function borrow_mut_(open_table: OpenTable, key: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): any {
  return Stdlib.Table.borrow_mut_(open_table.base_table, $.copy(key), $c, [$p[0], $p[1]]);
}

export function contains_(open_table: OpenTable, key: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): boolean {
  return Stdlib.Table.contains_(open_table.base_table, $.copy(key), $c, [$p[0], $p[1]]);
}

export function empty_($c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): OpenTable {
  return new OpenTable(
    { base_table: Stdlib.Table.new___($c, [$p[0], $p[1]]), keys: Stdlib.Vector.empty_($c, [$p[0]]) },
    new SimpleStructTag(OpenTable, [$p[0], $p[1]])
  );
}

export function loadParsers(repo: AptosParserRepo) {
  repo.addParser(
    '0xb1d4c0de8bc24468608637dfdbff975a0888f8935aa63338a44078eec5c7b6c7::open_table::OpenTable',
