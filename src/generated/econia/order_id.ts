...
export function counter_ask_(order_id: U128, $c: AptosDataCache): U64 {
  return u64($.copy(order_id).and(u128($.copy(HI_64))));
}

export function counter_bid_(order_id: U128, $c: AptosDataCache): U64 {
  return u64($.copy(order_id).and(u128($.copy(HI_64)))).xor($.copy(HI_64));
}

export function order_id_(price: U64, counter: U64, side: boolean, $c: AptosDataCache): U128 {
  let temp$1;
  if (side == $.copy(ASK)) {
...
export function order_id_ask_(price: U64, counter: U64, $c: AptosDataCache): U128 {
  return u128($.copy(price))
    .shl($.copy(FIRST_64))
    .or(u128($.copy(counter)));
}

export function order_id_bid_(price: U64, counter: U64, $c: AptosDataCache): U128 {
  return u128($.copy(price))
    .shl($.copy(FIRST_64))
    .or(u128($.copy(counter).xor($.copy(HI_64))));
}
