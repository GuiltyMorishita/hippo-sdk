...
export class CoinRegistry {
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    this.type_to_coin_info = proto[
      "type_to_coin_info"
    ] as Iterable_table.IterableTable;
  }

  static CoinRegistryParser(
    data: any,
    typeTag: TypeTag,
    repo: AptosParserRepo
  ): CoinRegistry {
...
export function add_extension_(
  ...
  if (
    !(
      Stdlib.Signer.address_of_(coin_owner, $c).hex() ===
      Stdlib.Type_info.account_address_(type_info, $c).hex()
    )
  ) {
    throw $.abortCode($.copy(E_COIN_OWNER_ONLY));
  }
  coin_info = Iterable_table.borrow_mut_(
...
export function add_to_list_(
  ...
  list = $c.borrow_global_mut<CoinList>(
  ...
  Iterable_table.add_(
    list.coin_types,
    $.copy(coin_type),
    new Nothing({}, new SimpleStructTag(Nothing)),
    $c,
    [
      new StructTag(new HexString("0x1"), "type_info", "TypeInfo", []),
...
export function fetch_all_registered_coin_info_(
  ...
  return $c.move_to(
    ...
    fetcher,
    get_all_registered_coin_info_($c)
  );
}
...
function make_query_fetch_all_registered_coin_info(app: App) {
  function maker(fetcher: $.SimulationKeys, $p: TypeTag[]) {
    return query_fetch_all_registered_coin_info(
      app.client,
      fetcher,
      app.repo,
      $p
...
export function fetch_full_list_(
  ...
  return $c.move_to(
    new SimpleStructTag(FullList),
    fetcher,
    get_full_list_($.copy(list_owner_addr), $c)
  );
}
...
export function get_all_registered_coin_info_($c: AptosDataCache): FullList {
  ...
  while (
    Stdlib.Option.is_some_(tail, $c, [
      new StructTag(new HexString("0x1"), "type_info", "TypeInfo", []),
    ])
  ) {
    {
      tail_key = $.copy(
        Stdlib.Option.borrow_(tail, $c, [
          new StructTag(new HexString("0x1"), "type_info", "TypeInfo", []),
        ])
      );
      coin_info = Iterable_table.borrow_(
        registry.type_to_coin_info,
        $.copy(tail_key),
        $c,
        [
      ...
      Stdlib.Vector.push_back_(fulllist.coin_info_list, $.copy(coin_info), $c, [
        new SimpleStructTag(CoinInfo),
      ]);
      [, prev] = Iterable_table.borrow_iter_(
        registry.type_to_coin_info,
        $.copy(tail_key),
        $c,
        [
...
export function get_full_list_(
  ...
  list = $c.borrow_global<CoinList>(
    new SimpleStructTag(CoinList),
    $.copy(list_owner_addr)
  );
  registry = $c.borrow_global<CoinRegistry>(
    new SimpleStructTag(CoinRegistry),
    new HexString(
...
export function is_coin_registered_(
  ...
  type_info = Stdlib.Type_info.type_of_($c, [$p[0]]);
  return Iterable_table.contains_(
    registry.type_to_coin_info,
    $.copy(type_info),
    $c,
    [
...
export function remove_from_list_(
  ...
  list = $c.borrow_global_mut<CoinList>(
    new SimpleStructTag(CoinList),
    Stdlib.Signer.address_of_(list_owner, $c)
  );
  coin_type = Stdlib.Type_info.type_of_($c, [$p[0]]);
...
export class App {
  ...
  async loadCoinRegistry(owner: HexString, loadFull = true) {
    const val = await CoinRegistry.load(
      this.repo,
      this.client,
      owner,
      [] as TypeTag[]
