...
export function init_coin_(
  ...
  [burn, freeze, mint] = Stdlib.Coin.initialize_(
    admin,
  ...
  $c.move_to(
    ...
    admin,
    new CoinCaps(
      { mint: $.copy(mint), freeze: $.copy(freeze), burn: $.copy(burn) },
      new SimpleStructTag(CoinCaps, [$p[0]])
    )
  );
  return;
...
export function init_coin_and_register_(
  ...
  init_coin_(
    admin,
    $.copy(Stdlib.String.bytes_(name, $c)),
    $.copy(Stdlib.String.bytes_(symbol, $c)),
    $.copy(decimals),
    $c,
    [$p[0]]
  );
...
export class App {
  ...
  async loadCoinCaps(owner: HexString, $p: TypeTag[] /* <T> */, loadFull = true, fillCache = true) {
  ...
  async deploy(_account: AptosAccount, option?: OptionTransaction, _isJSON = false) {
