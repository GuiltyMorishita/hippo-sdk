...
program.command('coin-list:add-to-list').description('').argument('<TYPE_CoinType>').action(coin_list_add_to_list);
...
const cp_scripts_remove_liquidity_script = async (
  ...
  const payload = Hippo_swap.Cp_scripts.buildPayload_remove_liquidity_script(liquidity_, amount_x_min_, amount_y_min_, [
    X_,
    Y_
  ]);
  await sendPayloadTx(client, account, payload);
...
const piece_swap_script_create_new_pool_script = async (
  ...
  const payload = Hippo_swap.Piece_swap_script.buildPayload_create_new_pool_script(
    lp_name_,
    lp_symbol_,
    k_,
    w1_numerator_,
...
const stable_curve_scripts_swap_script = async (
  ...
  const x_min_out_ = u64(x_min_out);
  const y_min_out_ = u64(y_min_out);
  const payload = Hippo_swap.Stable_curve_scripts.buildPayload_swap_script(x_in_, y_in_, x_min_out_, y_min_out_, [
    X_,
    Y_
  ]);
  await sendPayloadTx(client, account, payload);
...
program.command('volume:initialize').description('').argument('<poster>').action(volume_initialize);
...
program.command('volume:set-poster').description('').argument('<new_poster>').action(volume_set_poster);
...
program.command('coins:mint').description('').argument('<TYPE_CoinType>').argument('<amount>').action(coins_mint);
...
program.command('coin-list:query-fetch-full-list').argument('<list_owner_addr>').action(coin_list_fetch_full_list);

const utils_get_pool_list = async () => {
  const { client, account } = readConfig(program);
  const repo = getProjectRepo();
  const value = await Hippo_swap.Utils.query_get_pool_list(client, getSimulationKeys(account), repo, []);
  print(value);
};
...
const volume_fetch_volume = async () => {
  const { client, account } = readConfig(program);
  const repo = getProjectRepo();
  const value = await Hippo_aggregator.Volume.query_fetch_volume(client, getSimulationKeys(account), repo, []);
  print(value);
};
