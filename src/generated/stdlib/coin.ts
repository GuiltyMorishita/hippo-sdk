...
export class Coin {
  ...
  static typeParameters: TypeParamDeclType[] = [{ name: 'CoinType', isPhantom: true }];
  static fields: FieldDeclType[] = [{ name: 'value', typeTag: AtomicTypeTag.U64 }];
  ...
  static fields: FieldDeclType[] = [
    ...
    { name: 'decimals', typeTag: AtomicTypeTag.U8 },
    {
      ...
      typeTag: new StructTag(new HexString('0x1'), 'option', 'Option', [
        new StructTag(new HexString('0x1'), 'optional_aggregator', 'OptionalAggregator', [])
      ])
    }
  ];
...
export class DepositEvent {
  ...
  static fields: FieldDeclType[] = [{ name: 'amount', typeTag: AtomicTypeTag.U64 }];
...
export class SupplyConfig {
  ...
  static typeParameters: TypeParamDeclType[] = [];
  static fields: FieldDeclType[] = [{ name: 'allow_upgrades', typeTag: AtomicTypeTag.Bool }];
...
export function allow_supply_upgrades_(aptos_framework: HexString, allowed: boolean, $c: AptosDataCache): void {
  let allow_upgrades;
...
export function balance_(owner: HexString, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): U64 {
...
export function burn_(coin: Coin, _cap: BurnCapability, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
...
export function coin_address_($c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): HexString {
  let type_info;
  type_info = Type_info.type_of_($c, [$p[0]]);
  return Type_info.account_address_(type_info, $c);
}

export function decimals_($c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): U8 {
  return $.copy(
    $c.borrow_global<CoinInfo>(new SimpleStructTag(CoinInfo, [$p[0]]), coin_address_($c, [$p[0]])).decimals
  );
}

export function deposit_(account_addr: HexString, coin: Coin, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
  let coin_store;
  ...
  Event.emit_event_(
    ...
    new DepositEvent({ amount: $.copy(coin.value) }, new SimpleStructTag(DepositEvent)),
    $c,
...
export function destroy_burn_cap_(burn_cap: BurnCapability, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
  $.copy(burn_cap);
  return;
}
...
export function destroy_mint_cap_(mint_cap: MintCapability, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
  $.copy(mint_cap);
  return;
}

export function destroy_zero_(zero_coin: Coin, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
...
export function extract_(coin: Coin, amount: U64, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): Coin {
...
export function extract_all_(coin: Coin, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): Coin {
  let total_value;
...
export function initialize_(
  ...
  return initialize_internal_(account, $.copy(name), $.copy(symbol), $.copy(decimals), monitor_supply, false, $c, [
    $p[0]
  ]);
}
...
export function initialize_internal_(
  ...
  if (monitor_supply) {
    temp$1 = Option.some_(Optional_aggregator.new___($.copy(MAX_U128), parallelizable, $c), $c, [
      new StructTag(new HexString('0x1'), 'optional_aggregator', 'OptionalAggregator', [])
    ]);
  } else {
...
export function initialize_supply_config_(aptos_framework: HexString, $c: AptosDataCache): void {
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  $c.move_to(
    new SimpleStructTag(SupplyConfig),
    aptos_framework,
    new SupplyConfig({ allow_upgrades: false }, new SimpleStructTag(SupplyConfig))
  );
  return;
}
...
export function initialize_with_parallelizable_supply_(
  ...
  return initialize_internal_(account, $.copy(name), $.copy(symbol), $.copy(decimals), monitor_supply, true, $c, [
    $p[0]
  ]);
}
...
export function is_coin_initialized_($c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): boolean {
  return $c.exists(new SimpleStructTag(CoinInfo, [$p[0]]), coin_address_($c, [$p[0]]));
}

export function merge_(dst_coin: Coin, source_coin: Coin, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
  dst_coin.value = $.copy(dst_coin.value).add($.copy(source_coin.value));
  source_coin;
  return;
}

export function mint_(amount: U64, _cap: MintCapability, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): Coin {
  let maybe_supply, supply;
  if ($.copy(amount).eq(u64('0'))) {
    return zero_($c, [$p[0]]);
  } else {
...
export function name_($c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): String.String {
  return $.copy($c.borrow_global<CoinInfo>(new SimpleStructTag(CoinInfo, [$p[0]]), coin_address_($c, [$p[0]])).name);
}

export function register_(account: HexString, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
  ...
  coin_store = new CoinStore(
    {
      ...
      frozen: false,
      deposit_events: Account.new_event_handle_(account, $c, [new SimpleStructTag(DepositEvent)]),
      withdraw_events: Account.new_event_handle_(account, $c, [new SimpleStructTag(WithdrawEvent)])
    },
...
export function supply_($c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): Option.Option {
...
export function symbol_($c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): String.String {
...
export function buildPayload_transfer(
  ...
  return $.buildPayload(new HexString('0x1'), 'coin', 'transfer', typeParamStrings, [to, amount], isJSON);
}
export function upgrade_supply_(account: HexString, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
  ...
  if (
    !$.copy($c.borrow_global_mut<SupplyConfig>(new SimpleStructTag(SupplyConfig), new HexString('0x1')).allow_upgrades)
  ) {
...
export function buildPayload_upgrade_supply(
  ...
  const typeParamStrings = $p.map((t) => $.getTypeTagFullname(t));
  return $.buildPayload(new HexString('0x1'), 'coin', 'upgrade_supply', typeParamStrings, [], isJSON);
}
export function value_(coin: Coin, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): U64 {
  return $.copy(coin.value);
}

export function withdraw_(account: HexString, amount: U64, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): Coin {
  let account_addr, coin_store;
  account_addr = Signer.address_of_(account, $c);
  if (!is_account_registered_($.copy(account_addr), $c, [$p[0]])) {
    throw $.abortCode(Error.not_found_($.copy(ECOIN_STORE_NOT_PUBLISHED), $c));
  ...
  Event.emit_event_(
    ...
    new WithdrawEvent({ amount: $.copy(amount) }, new SimpleStructTag(WithdrawEvent)),
    $c,
