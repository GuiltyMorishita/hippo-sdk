...
export class CurrentTimeMicroseconds {
  ...
  static fields: FieldDeclType[] = [{ name: 'microseconds', typeTag: AtomicTypeTag.U64 }];
...
export function now_microseconds_($c: AptosDataCache): U64 {
  return $.copy(
    $c.borrow_global<CurrentTimeMicroseconds>(new SimpleStructTag(CurrentTimeMicroseconds), new HexString('0x1'))
      .microseconds
  );
}
...
export function set_time_has_started_(aptos_framework: HexString, $c: AptosDataCache): void {
  let timer;
...
export function update_global_time_(account: HexString, proposer: HexString, timestamp: U64, $c: AptosDataCache): void {
