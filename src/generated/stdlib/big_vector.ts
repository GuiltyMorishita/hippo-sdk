...
export class BigVector {
  ...
  static typeParameters: TypeParamDeclType[] = [{ name: 'T', isPhantom: false }];
  static fields: FieldDeclType[] = [
    {
      name: 'buckets',
      typeTag: new StructTag(new HexString('0x1'), 'table_with_length', 'TableWithLength', [
        AtomicTypeTag.U64,
        new VectorTag(new $.TypeParamIdx(0))
      ])
    },
...
export function borrow_(v: BigVector, index: BigVectorIndex, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): any {
  return Vector.borrow_(
    Table_with_length.borrow_(v.buckets, $.copy(index.bucket_index), $c, [AtomicTypeTag.U64, new VectorTag($p[0])]),
    $.copy(index.vec_index),
    $c,
    [$p[0]]
  );
}

export function borrow_mut_(v: BigVector, index: BigVectorIndex, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): any {
  return Vector.borrow_mut_(
    Table_with_length.borrow_mut_(v.buckets, $.copy(index.bucket_index), $c, [AtomicTypeTag.U64, new VectorTag($p[0])]),
    $.copy(index.vec_index),
    $c,
    [$p[0]]
  );
...
export function bucket_index_(v: BigVector, i: U64, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): BigVectorIndex {
  if (!$.copy(i).lt(length_(v, $c, [$p[0]]))) {
    throw $.abortCode($.copy(EINDEX_OUT_OF_BOUNDS));
  }
  return new BigVectorIndex(
    { bucket_index: $.copy(i).div($.copy(v.bucket_size)), vec_index: $.copy(i).mod($.copy(v.bucket_size)) },
    new SimpleStructTag(BigVectorIndex)
  );
}

export function bucket_size_(v: BigVector, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): U64 {
  return $.copy(v.bucket_size);
}

export function buckets_required_(end_index: BigVectorIndex, $c: AptosDataCache): U64 {
...
export function contains_(v: BigVector, val: any, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): boolean {
  let exist;
...
export function decrement_index_(index: BigVectorIndex, bucket_size: U64, $c: AptosDataCache): void {
...
export function destroy_empty_(v: BigVector, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): void {
  if (!is_empty_(v, $c, [$p[0]])) {
...
export function increment_index_(index: BigVectorIndex, bucket_size: U64, $c: AptosDataCache): void {
...
export function index_of_(v: BigVector, val: any, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): [boolean, U64] {
  let i, index, len;
  i = u64('0');
  len = length_(v, $c, [$p[0]]);
...
export function is_empty_(v: BigVector, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): boolean {
  return length_(v, $c, [$p[0]]).eq(u64('0'));
}

export function length_(v: BigVector, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): U64 {
  return $.copy(v.end_index.bucket_index).mul($.copy(v.bucket_size)).add($.copy(v.end_index.vec_index));
}

export function new___(bucket_size: U64, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): BigVector {
  if (!$.copy(bucket_size).gt(u64('0'))) {
    throw $.abortCode(u64('0'));
  }
  return new BigVector(
    {
      buckets: Table_with_length.new___($c, [AtomicTypeTag.U64, new VectorTag($p[0])]),
      end_index: new BigVectorIndex(
        { bucket_index: u64('0'), vec_index: u64('0') },
        new SimpleStructTag(BigVectorIndex)
      ),
      num_buckets: u64('0'),
      bucket_size: $.copy(bucket_size)
    },
    new SimpleStructTag(BigVector, [$p[0]])
  );
}
...
export function pop_back_(v: BigVector, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): any {
  let val;
  if (!!is_empty_(v, $c, [$p[0]])) {
    throw $.abortCode(Error.invalid_argument_($.copy(EINDEX_OUT_OF_BOUNDS), $c));
  }
  decrement_index_(v.end_index, $.copy(v.bucket_size), $c);
  val = Vector.pop_back_(
    Table_with_length.borrow_mut_(v.buckets, $.copy(v.end_index.bucket_index), $c, [
      AtomicTypeTag.U64,
      new VectorTag($p[0])
    ]),
    $c,
...
export function push_back_(v: BigVector, val: any, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): void {
  if ($.copy(v.end_index.bucket_index).eq($.copy(v.num_buckets))) {
    Table_with_length.add_(v.buckets, $.copy(v.num_buckets), Vector.empty_($c, [$p[0]]), $c, [
      AtomicTypeTag.U64,
      new VectorTag($p[0])
    ]);
...
export function push_back_no_grow_(v: BigVector, val: any, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): void {
...
export function reserve_(v: BigVector, additional_buckets: U64, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): void {
...
export function shrink_to_fit_(v: BigVector, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): void {
  let v__1;
...
export function swap_remove_(v: BigVector, index: BigVectorIndex, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): any {
  ...
  bucket = Table_with_length.borrow_mut_(v.buckets, $.copy(index.bucket_index), $c, [
    AtomicTypeTag.U64,
    new VectorTag($p[0])
  ]);
