  ...
  static fields: FieldDeclType[] = [
    {
      ...
      typeTag: new StructTag(
        ...
        "table_with_length",
        "TableWithLength",
        [AtomicTypeTag.U64, new VectorTag(new $.TypeParamIdx(0))]
      ),
    },
    {
      name: "end_index",
...
export function borrow_(
  ...
  return Vector.borrow_(
    Table_with_length.borrow_(v.buckets, $.copy(index.bucket_index), $c, [
      AtomicTypeTag.U64,
      new VectorTag($p[0]),
    ]),
...
export function borrow_mut_(
  ...
  return Vector.borrow_mut_(
    Table_with_length.borrow_mut_(v.buckets, $.copy(index.bucket_index), $c, [
      AtomicTypeTag.U64,
      new VectorTag($p[0]),
    ]),
    $.copy(index.vec_index),
...
export function bucket_index_(
  ...
  return new BigVectorIndex(
    {
      bucket_index: $.copy(i).div($.copy(v.bucket_size)),
      vec_index: $.copy(i).mod($.copy(v.bucket_size)),
    },
...
export function length_(
  ...
  return $.copy(v.end_index.bucket_index)
    .mul($.copy(v.bucket_size))
    .add($.copy(v.end_index.vec_index));
}
...
export function pop_back_(
  ...
  if (!!is_empty_(v, $c, [$p[0]])) {
    throw $.abortCode(
      Error.invalid_argument_($.copy(EINDEX_OUT_OF_BOUNDS), $c)
    );
  }
  decrement_index_(v.end_index, $.copy(v.bucket_size), $c);
  val = Vector.pop_back_(
    Table_with_length.borrow_mut_(
      v.buckets,
      $.copy(v.end_index.bucket_index),
      $c,
      [AtomicTypeTag.U64, new VectorTag($p[0])]
    ),
    $c,
    [$p[0]]
  );
  ...
  while ($.copy(v.num_buckets).gt(buckets_required_(v.end_index, $c))) {
    {
      ...
      v__1 = Table_with_length.remove_(v.buckets, $.copy(v.num_buckets), $c, [
        AtomicTypeTag.U64,
        new VectorTag($p[0]),
      ]);
...
export function swap_remove_(
  ...
  bucket = Table_with_length.borrow_mut_(
    v.buckets,
    $.copy(index.bucket_index),
    $c,
    [AtomicTypeTag.U64, new VectorTag($p[0])]
  );
...
export function loadParsers(repo: AptosParserRepo) {
  ...
  repo.addParser(
    "0x1::big_vector::BigVectorIndex",
    BigVectorIndex.BigVectorIndexParser
  );
}
export class App {
  constructor(
