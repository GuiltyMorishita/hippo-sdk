...
export function add_stake_(
  ...
  add_stake_with_cap_(
    ownership_cap,
    Coin.withdraw_(account, $.copy(amount), $c, [
      new StructTag(new HexString("0x1"), "aptos_coin", "AptosCoin", []),
    ]),
    $c
  );
...
export function add_stake_with_cap_(
  ...
  if (!$.copy(amount).gt(u64("0"))) {
    throw $.abortCode(
      Error.invalid_argument_($.copy(EINVALID_STAKE_AMOUNT), $c)
    );
  }
...
export function join_validator_set_internal_(
  ...
  if (
    !(
      Signer.address_of_(account, $c).hex() ===
  ...
  if (
    ...
    throw $.abortCode(
      Error.invalid_argument_($.copy(EALREADY_ACTIVE_VALIDATOR), $c)
    );
  }
  config = Staking_config.get_($c);
  ...
  if (
    !!Vector.is_empty_(validator_config.consensus_pubkey, $c, [
      AtomicTypeTag.U8,
    ])
  ) {
    throw $.abortCode(Error.invalid_argument_($.copy(EINVALID_PUBLIC_KEY), $c));
...
export function withdraw_(
  ...
  Coin.deposit_($.copy(account_addr), coins, $c, [
    new StructTag(new HexString("0x1"), "aptos_coin", "AptosCoin", []),
  ]);
  return;
}
...
export function withdraw_with_cap_(
  ...
  if (!$.copy(withdraw_amount).gt(u64("0"))) {
    throw $.abortCode(
      Error.invalid_argument_($.copy(ENO_COINS_TO_WITHDRAW), $c)
    );
  }
  Event.emit_event_(
...
export class App {
  ...
  async initialize_stake_owner(
    ...
    const payload = buildPayload_initialize_stake_owner(
      initial_stake_amount,
      operator,
      voter,
      _isJSON
  ...
  async rotate_consensus_key(
    ...
    const payload = buildPayload_rotate_consensus_key(
      pool_address,
      new_consensus_pubkey,
      proof_of_possession,
      _isJSON
  ...
  async set_delegated_voter(
    ...
    const payload = buildPayload_set_delegated_voter(
      new_delegated_voter,
      _isJSON
    );
    return $.sendPayloadTx(this.client, _account, payload, _maxGas);
