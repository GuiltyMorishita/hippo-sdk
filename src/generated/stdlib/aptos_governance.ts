  ...
  static fields: FieldDeclType[] = [
    {
      ...
      typeTag: new StructTag(new HexString('0x1'), 'event', 'EventHandle', [
        new StructTag(new HexString('0x1'), 'aptos_governance', 'CreateProposalEvent', [])
      ])
    },
    {
      name: 'update_config_events',
      typeTag: new StructTag(new HexString('0x1'), 'event', 'EventHandle', [
        new StructTag(new HexString('0x1'), 'aptos_governance', 'UpdateConfigEvent', [])
      ])
    },
    {
      name: 'vote_events',
      typeTag: new StructTag(new HexString('0x1'), 'event', 'EventHandle', [
        new StructTag(new HexString('0x1'), 'aptos_governance', 'VoteEvent', [])
      ])
    }
  ];
  ...
  static fields: FieldDeclType[] = [
    {
      ...
      typeTag: new StructTag(new HexString('0x1'), 'table', 'Table', [
        new StructTag(new HexString('0x1'), 'aptos_governance', 'RecordKey', []),
        AtomicTypeTag.Bool
      ])
    }
  ];
...
export function add_approved_script_hash_(proposal_id: U64, $c: AptosDataCache): void {
  ...
  proposal_state = Voting.get_proposal_state_(new HexString('0x1'), $.copy(proposal_id), $c, [
    new StructTag(new HexString('0x1'), 'governance_proposal', 'GovernanceProposal', [])
  ]);
  ...
  execution_hash = Voting.get_execution_hash_(new HexString('0x1'), $.copy(proposal_id), $c, [
    new StructTag(new HexString('0x1'), 'governance_proposal', 'GovernanceProposal', [])
  ]);
  Simple_map.add_(approved_hashes.hashes, $.copy(proposal_id), $.copy(execution_hash), $c, [
    AtomicTypeTag.U64,
    new VectorTag(AtomicTypeTag.U8)
  ]);
  return;
...
export function add_approved_script_hash_script_(proposal_id: U64, $c: AptosDataCache): void {
...
export function create_proposal_(
  ...
  if (Option.is_some_(total_voting_token_supply, $c, [AtomicTypeTag.U128])) {
    ...
    early_resolution_vote_threshold = Option.some_($.copy(total_supply).div(u128('2')).add(u128('1')), $c, [
      AtomicTypeTag.U128
    ]);
  } else {
  }
  ...
  Event.emit_event_(
    ...
    new CreateProposalEvent(
      { proposer: temp$2, stake_pool: temp$3, proposal_id: temp$1, execution_hash: temp$4, proposal_metadata: temp$5 },
...
export function create_proposal_metadata_(
  ...
  Simple_map.add_(metadata, String.utf8_($.copy(METADATA_LOCATION_KEY), $c), $.copy(metadata_location), $c, [
    new StructTag(new HexString('0x1'), 'string', 'String', []),
    new VectorTag(AtomicTypeTag.U8)
  ]);
  Simple_map.add_(metadata, String.utf8_($.copy(METADATA_HASH_KEY), $c), $.copy(metadata_hash), $c, [
    new StructTag(new HexString('0x1'), 'string', 'String', []),
    new VectorTag(AtomicTypeTag.U8)
  ]);
...
export function get_min_voting_threshold_($c: AptosDataCache): U128 {
  return $.copy(
    $c.borrow_global<GovernanceConfig>(new SimpleStructTag(GovernanceConfig), new HexString('0x1')).min_voting_threshold
  );
}

export function get_required_proposer_stake_($c: AptosDataCache): U64 {
  return $.copy(
    $c.borrow_global<GovernanceConfig>(new SimpleStructTag(GovernanceConfig), new HexString('0x1'))
      .required_proposer_stake
  );
}

export function get_signer_(signer_address: HexString, $c: AptosDataCache): HexString {
  ...
  signer_cap = Simple_map.borrow_(governance_responsibility.signer_caps, signer_address, $c, [
    AtomicTypeTag.Address,
    new StructTag(new HexString('0x1'), 'account', 'SignerCapability', [])
  ]);
...
export function get_voting_duration_secs_($c: AptosDataCache): U64 {
  return $.copy(
    $c.borrow_global<GovernanceConfig>(new SimpleStructTag(GovernanceConfig), new HexString('0x1')).voting_duration_secs
  );
}

export function get_voting_power_(pool_address: HexString, $c: AptosDataCache): U64 {
...
export function initialize_(
  ...
  $c.move_to(
    ...
    new GovernanceConfig(
      { min_voting_threshold: temp$2, required_proposer_stake: temp$3, voting_duration_secs: temp$1 },
      new SimpleStructTag(GovernanceConfig)
    )
  );
  $c.move_to(
    ...
    new GovernanceEvents(
      {
        ...
        update_config_events: Account.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(UpdateConfigEvent)]),
        vote_events: Account.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(VoteEvent)])
      },
      new SimpleStructTag(GovernanceEvents)
    )
  );
  $c.move_to(
    ...
    aptos_framework,
    new VotingRecords(
      { votes: Table.new___($c, [new SimpleStructTag(RecordKey), AtomicTypeTag.Bool]) },
      new SimpleStructTag(VotingRecords)
    )
  );
  return $c.move_to(
    ...
    new ApprovedExecutionHashes(
      { hashes: Simple_map.create_($c, [AtomicTypeTag.U64, new VectorTag(AtomicTypeTag.U8)]) },
      new SimpleStructTag(ApprovedExecutionHashes)
    )
  );
}
...
export function reconfigure_(aptos_framework: HexString, $c: AptosDataCache): void {
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  Reconfiguration.reconfigure_($c);
  return;
}

export function remove_approved_hash_(proposal_id: U64, $c: AptosDataCache): void {
...
export function resolve_(proposal_id: U64, signer_address: HexString, $c: AptosDataCache): HexString {
...
export function vote_(
  ...
  proposal_expiration = Voting.get_proposal_expiration_secs_(new HexString('0x1'), $.copy(proposal_id), $c, [
    new StructTag(new HexString('0x1'), 'governance_proposal', 'GovernanceProposal', [])
  ]);
  if (!Stake.get_lockup_secs_($.copy(stake_pool), $c).ge($.copy(proposal_expiration))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EINSUFFICIENT_STAKE_LOCKUP), $c));
  ...
  Voting.vote_(temp$1, new HexString('0x1'), $.copy(proposal_id), $.copy(voting_power), should_pass, $c, [
    new StructTag(new HexString('0x1'), 'governance_proposal', 'GovernanceProposal', [])
  ]);
  events = $c.borrow_global_mut<GovernanceEvents>(new SimpleStructTag(GovernanceEvents), new HexString('0x1'));
  Event.emit_event_(
