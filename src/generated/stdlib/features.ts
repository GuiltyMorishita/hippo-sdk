...
export class Features {
  ...
  static fields: FieldDeclType[] = [{ name: 'features', typeTag: new VectorTag(AtomicTypeTag.U8) }];
...
export function change_feature_flags_(framework: HexString, enable: U64[], disable: U64[], $c: AptosDataCache): void {
...
export function contains_(features: U8[], feature: U64, $c: AptosDataCache): boolean {
  ...
  if ($.copy(byte_index).lt(Vector.length_(features, $c, [AtomicTypeTag.U8]))) {
    temp$1 = $.copy(Vector.borrow_(features, $.copy(byte_index), $c, [AtomicTypeTag.U8]))
      .and($.copy(bit_mask))
      .neq(u8('0'));
  } else {
    temp$1 = false;
...
export function is_enabled_(feature: U64, $c: AptosDataCache): boolean {
  ...
  if ($c.exists(new SimpleStructTag(Features), new HexString('0x1'))) {
    temp$1 = contains_(
      $c.borrow_global<Features>(new SimpleStructTag(Features), new HexString('0x1')).features,
      $.copy(feature),
      $c
    );
  } else {
...
export function set_(features: U8[], feature: U64, include: boolean, $c: AptosDataCache): void {
