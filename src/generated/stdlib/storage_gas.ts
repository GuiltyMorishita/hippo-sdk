...
export function base_8192_exponential_curve_(min_gas: U64, max_gas: U64, $c: AptosDataCache): GasCurve {
...
export function calculate_create_gas_(config: UsageGasConfig, usage: U64, $c: AptosDataCache): U64 {
  return calculate_gas_($.copy(config.target_usage), $.copy(usage), config.create_curve, $c);
}

export function calculate_gas_(max_usage: U64, current_usage: U64, curve: GasCurve, $c: AptosDataCache): U64 {
  let temp$1,
    temp$10,
    temp$11,
    temp$12,
  ...
  if ($.copy(num_points).eq(u64('0'))) {
    ...
    temp$4 = temp$3;
    temp$2 = new Point(
      { x: $.copy(BASIS_POINT_DENOMINATION), y: $.copy(BASIS_POINT_DENOMINATION) },
    ...
    while ($.copy(i).lt($.copy(j))) {
      {
        ...
        if (
          $.copy(current_usage_bps).lt($.copy(Vector.borrow_(points, $.copy(mid), $c, [new SimpleStructTag(Point)]).x))
        ) {
  ...
  return $.copy(curve.min_gas).add(
    $.copy(curve.max_gas)
      .sub($.copy(curve.min_gas))
      .mul(
        $.copy(left.y).add(
          $.copy(current_usage_bps)
            .sub($.copy(left.x))
            .mul($.copy(right.y).sub($.copy(left.y)))
            .div($.copy(right.x).sub($.copy(left.x)))
        )
      )
      .div($.copy(BASIS_POINT_DENOMINATION))
  );
}

export function calculate_read_gas_(config: UsageGasConfig, usage: U64, $c: AptosDataCache): U64 {
  return calculate_gas_($.copy(config.target_usage), $.copy(usage), config.read_curve, $c);
}

export function calculate_write_gas_(config: UsageGasConfig, usage: U64, $c: AptosDataCache): U64 {
  return calculate_gas_($.copy(config.target_usage), $.copy(usage), config.write_curve, $c);
}

export function init_module_(aptos_framework: HexString, $c: AptosDataCache): void {
...
export function initialize_(aptos_framework: HexString, $c: AptosDataCache): void {
  ...
  item_config = new UsageGasConfig(
    {
      target_usage: u64('1000000000'),
      read_curve: base_8192_exponential_curve_(u64('80000'), u64('80000').mul(u64('100')), $c),
      create_curve: base_8192_exponential_curve_(u64('2000000'), u64('2000000').mul(u64('100')), $c),
      write_curve: base_8192_exponential_curve_(u64('400000'), u64('400000').mul(u64('100')), $c)
    },
    new SimpleStructTag(UsageGasConfig)
  );
  byte_config = new UsageGasConfig(
    {
      target_usage: u64('500000000000'),
      read_curve: base_8192_exponential_curve_(u64('40'), u64('40').mul(u64('100')), $c),
      create_curve: base_8192_exponential_curve_(u64('1000'), u64('1000').mul(u64('100')), $c),
      write_curve: base_8192_exponential_curve_(u64('200'), u64('200').mul(u64('100')), $c)
    },
    new SimpleStructTag(UsageGasConfig)
  );
  ...
  $c.move_to(
    ...
    new StorageGas(
      {
        per_item_read: u64('8000'),
        per_item_create: u64('1280000'),
        per_item_write: u64('160000'),
        per_byte_read: u64('1000'),
        per_byte_create: u64('10000'),
        per_byte_write: u64('10000')
      },
      new SimpleStructTag(StorageGas)
    )
  );
...
export function new_gas_curve_(min_gas: U64, max_gas: U64, points: Point[], $c: AptosDataCache): GasCurve {
  return new GasCurve(
    { min_gas: $.copy(min_gas), max_gas: $.copy(max_gas), points: $.copy(points) },
...
export function set_config_(aptos_framework: HexString, config: StorageGasConfig, $c: AptosDataCache): void {
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  validate_usage_config_(config.item_config, $c);
  validate_usage_config_(config.byte_config, $c);
  $.set(
    $c.borrow_global_mut<StorageGasConfig>(new SimpleStructTag(StorageGasConfig), new HexString('0x1')),
    $.copy(config)
  );
  return;
}

export function validate_curve_(curve: GasCurve, $c: AptosDataCache): void {
...
export function validate_usage_config_(config: UsageGasConfig, $c: AptosDataCache): void {
