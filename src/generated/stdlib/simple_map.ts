  ...
  static fields: FieldDeclType[] = [
    {
      ...
      typeTag: new VectorTag(
        new StructTag(new HexString('0x1'), 'simple_map', 'Element', [new $.TypeParamIdx(0), new $.TypeParamIdx(1)])
      )
    }
  ];
...
export function add_(map: SimpleMap, key: any, value: any, $c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): void {
  ...
  Vector.push_back_(
    map.data,
    new Element({ key: key, value: value }, new SimpleStructTag(Element, [$p[0], $p[1]])),
    $c,
...
export function borrow_(map: SimpleMap, key: any, $c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): any {
  let idx, maybe_idx;
...
export function borrow_mut_(map: SimpleMap, key: any, $c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): any {
...
export function contains_key_(map: SimpleMap, key: any, $c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): boolean {
  let maybe_idx;
  [maybe_idx] = find_(map, key, $c, [$p[0], $p[1]]);
  return Option.is_some_(maybe_idx, $c, [AtomicTypeTag.U64]);
}

export function create_($c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): SimpleMap {
...
export function destroy_empty_(map: SimpleMap, $c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): void {
  let { data: data } = map;
...
export function find_(
  ...
  if ($.copy(left).neq($.copy(length))) {
    temp$2 = $.dyn_eq(
      $p[0],
      key,
      Vector.borrow_(map.data, $.copy(left), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]).key
    );
  } else {
    temp$2 = false;
  }
...
export function length_(map: SimpleMap, $c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): U64 {
  return Vector.length_(map.data, $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
}

export function remove_(map: SimpleMap, key: any, $c: AptosDataCache, $p: TypeTag[] /* <Key, Value>*/): [any, any] {
  ...
  while ($.copy(placement).lt($.copy(end))) {
    {
      Vector.swap_(map.data, $.copy(placement), $.copy(placement).add(u64('1')), $c, [
        new SimpleStructTag(Element, [$p[0], $p[1]])
      ]);
      placement = $.copy(placement).add(u64('1'));
    }
