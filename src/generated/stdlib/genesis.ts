...
export class EmployeeAccountMap {
  ...
  static fields: FieldDeclType[] = [
    ...
    {
      name: 'validator',
      typeTag: new StructTag(new HexString('0x1'), 'genesis', 'ValidatorConfigurationWithCommission', [])
    },
...
export function create_accounts_(aptos_framework: HexString, accounts: AccountMap[], $c: AptosDataCache): void {
...
export function create_employee_validators_(employees: EmployeeAccountMap[], $c: AptosDataCache): void {
  let temp$2,
    account,
    admin,
    admin_signer,
  ...
  while ($.copy(i).lt($.copy(num_employee_groups))) {
    {
      ...
      vesting_schedule = Vesting.create_vesting_schedule_(
        $.copy(schedule),
        u64('1663456089'),
        u64('30').mul(u64('24')).mul(u64('60')).mul(u64('60')),
        $c
      );
  ...
  while ($.copy(i).lt($.copy(num_validators))) {
    {
      validator_with_commission = new ValidatorConfigurationWithCommission(
        {
          validator_config: Vector.pop_back_(validators, $c, [new SimpleStructTag(ValidatorConfiguration)]),
          commission_percentage: u64('0'),
          join_during_genesis: true
        },
        new SimpleStructTag(ValidatorConfigurationWithCommission)
      );
      Vector.push_back_(validators_with_commission, $.copy(validator_with_commission), $c, [
        new SimpleStructTag(ValidatorConfigurationWithCommission)
      ]);
      i = $.copy(i).add(u64('1'));
    }
...
export function initialize_aptos_coin_(aptos_framework: HexString, $c: AptosDataCache): void {
