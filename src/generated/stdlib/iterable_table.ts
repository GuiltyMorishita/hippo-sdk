  ...
  static fields: FieldDeclType[] = [
    {
      ...
      typeTag: new StructTag(new HexString('0x1'), 'table_with_length', 'TableWithLength', [
        new $.TypeParamIdx(0),
        new StructTag(new HexString('0x1'), 'iterable_table', 'IterableValue', [
          new $.TypeParamIdx(0),
          new $.TypeParamIdx(1)
        ])
      ])
...
export function head_key_(table: IterableTable, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): Option.Option {
...
export function new___($c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): IterableTable {
  return new IterableTable(
    {
      inner: Table_with_length.new___($c, [$p[0], new SimpleStructTag(IterableValue, [$p[0], $p[1]])]),
...
export function tail_key_(table: IterableTable, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): Option.Option {
...
export class TypedIterableTable<K = any, V = any> {
  ...
  constructor(public table: IterableTable, public keyTag: TypeTag, public valueTag: TypeTag) {
  ...
  async fetchAll(client: AptosClient, repo: AptosParserRepo, cache: AptosLocalCache | null = null): Promise<[K, V][]> {
    ...
    while (next && (await Option.is_some_(next, dummyCache, [this.keyTag]))) {
      ...
      if (cache) {
        ...
        Table_with_length.add_(this.table.inner, $.copy(key), iterVal, cache, [$p[0], iterVal.typeTag]);
      }
    }
    return result;
  }
