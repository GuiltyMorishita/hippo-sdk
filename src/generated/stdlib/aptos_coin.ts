...
export class DelegatedMintCapability {
  ...
  static typeParameters: TypeParamDeclType[] = [];
  static fields: FieldDeclType[] = [{ name: 'to', typeTag: AtomicTypeTag.Address }];
...
export class Delegations {
  ...
  static fields: FieldDeclType[] = [
    {
      name: 'inner',
      typeTag: new VectorTag(new StructTag(new HexString('0x1'), 'aptos_coin', 'DelegatedMintCapability', []))
    }
  ];
...
export function claim_mint_capability_(account: HexString, $c: AptosDataCache): void {
  ...
  mint_cap = $.copy(
    $c.borrow_global<MintCapStore>(new SimpleStructTag(MintCapStore), new HexString('0xa550c18')).mint_cap
  );
  $c.move_to(
    new SimpleStructTag(MintCapStore),
    account,
    new MintCapStore({ mint_cap: $.copy(mint_cap) }, new SimpleStructTag(MintCapStore))
  );
  return;
}
...
export function buildPayload_claim_mint_capability(
  ...
  return $.buildPayload(new HexString('0x1'), 'aptos_coin', 'claim_mint_capability', typeParamStrings, [], isJSON);
}
export function configure_accounts_for_test_(
  ...
  $c.move_to(
    ...
    core_resources,
    new Delegations(
      { inner: Vector.empty_($c, [new SimpleStructTag(DelegatedMintCapability)]) },
      new SimpleStructTag(Delegations)
    )
  );
  return;
...
export function delegate_mint_capability_(account: HexString, to: HexString, $c: AptosDataCache): void {
  ...
  Vector.push_back_(
    delegations,
    new DelegatedMintCapability({ to: $.copy(to) }, new SimpleStructTag(DelegatedMintCapability)),
    $c,
...
export function buildPayload_delegate_mint_capability(
  ...
  const typeParamStrings = [] as string[];
  return $.buildPayload(new HexString('0x1'), 'aptos_coin', 'delegate_mint_capability', typeParamStrings, [to], isJSON);
}
export function destroy_mint_cap_(aptos_framework: HexString, $c: AptosDataCache): void {
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
...
export function find_delegation_(addr: HexString, $c: AptosDataCache): Option.Option {
...
export function has_mint_capability_(account: HexString, $c: AptosDataCache): boolean {
...
export function initialize_(
  ...
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  [burn_cap, freeze_cap, mint_cap] = Coin.initialize_with_parallelizable_supply_(
    aptos_framework,
    String.utf8_(
      [u8('65'), u8('112'), u8('116'), u8('111'), u8('115'), u8('32'), u8('67'), u8('111'), u8('105'), u8('110')],
      $c
    ),
...
export function mint_(account: HexString, dst_addr: HexString, amount: U64, $c: AptosDataCache): void {
...
export function buildPayload_mint(
  ...
  const typeParamStrings = [] as string[];
  return $.buildPayload(new HexString('0x1'), 'aptos_coin', 'mint', typeParamStrings, [dst_addr, amount], isJSON);
}
...
export class App {
  ...
  async claim_mint_capability(_account: AptosAccount, _maxGas = 1000, _isJSON = false) {
  ...
  async delegate_mint_capability(_account: AptosAccount, to: HexString, _maxGas = 1000, _isJSON = false) {
  ...
  async mint(_account: AptosAccount, dst_addr: HexString, amount: U64, _maxGas = 1000, _isJSON = false) {
