...
export function buildPayload_create_account(
  ...
  return $.buildPayload(new HexString('0x1'), 'aptos_account', 'create_account', typeParamStrings, [auth_key], isJSON);
}
export function transfer_(source: HexString, to: HexString, amount: U64, $c: AptosDataCache): void {
...
export function buildPayload_transfer(
  ...
  const typeParamStrings = [] as string[];
  return $.buildPayload(new HexString('0x1'), 'aptos_account', 'transfer', typeParamStrings, [to, amount], isJSON);
}
