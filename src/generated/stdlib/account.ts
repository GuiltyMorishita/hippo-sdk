...
export function create_authorized_signer_(
  ...
  if (
    ...
    throw $.abortCode(
      Error.not_found_($.copy(EINVALID_ACCEPT_SIGNER_CAPABILITY), $c)
    );
  }
...
export function offer_signer_capability_(
  ...
  if ($.copy(account_scheme).eq($.copy(ED25519_SCHEME))) {
    ...
    if ($.copy(account_scheme).eq($.copy(MULTI_ED25519_SCHEME))) {
      pubkey__2 = Multi_ed25519.new_unvalidated_public_key_from_bytes_(
        $.copy(account_public_key_bytes),
        $c
      );
      expected_auth_key__3 =
      ...
      signer_capability_sig__4 = Multi_ed25519.new_signature_from_bytes_(
        $.copy(signer_capability_sig_bytes),
        $c
      );
      if (
...
export function revoke_signer_capability_(
  ...
  if (
    ...
    throw $.abortCode(
      Error.not_found_($.copy(EINVALID_REVOKE_SIGNER_CAPABILITY), $c)
    );
  }
...
export function rotate_authentication_key_(
  ...
  curr_auth_key = From_bcs.to_address_(
    $.copy(account_resource.authentication_key),
    $c
  );
  ...
  if (
    Table.contains_(temp$4, temp$5, $c, [
      AtomicTypeTag.Address,
      AtomicTypeTag.Address,
    ])
  ) {
    if (
      !(
        $.copy(addr).hex() ===
        Table.remove_(address_map, $.copy(curr_address), $c, [
          AtomicTypeTag.Address,
          AtomicTypeTag.Address,
        ]).hex()
      )
  ...
  Table.add_(address_map, $.copy(new_address), $.copy(addr), $c, [
    AtomicTypeTag.Address,
    AtomicTypeTag.Address,
  ]);
  account_resource__6 = $c.borrow_global_mut<Account>(
    new SimpleStructTag(Account),
    $.copy(addr)
  );
  Event.emit_event_(
...
export class App {
  ...
  async revoke_signer_capability(
    ...
    const payload = buildPayload_revoke_signer_capability(
      to_be_revoked_address,
      _isJSON
    );
    return $.sendPayloadTx(this.client, _account, payload, _maxGas);
