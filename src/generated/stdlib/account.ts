...
export class Account {
  ...
  static fields: FieldDeclType[] = [
    ...
    },
    {
      ...
      typeTag: new StructTag(new HexString('0x1'), 'account', 'CapabilityOffer', [
        new StructTag(new HexString('0x1'), 'account', 'RotationCapability', [])
      ])
    },
    {
...
export class OriginatingAddress {
  ...
  static fields: FieldDeclType[] = [
    {
      name: 'address_map',
      typeTag: new StructTag(new HexString('0x1'), 'table', 'Table', [AtomicTypeTag.Address, AtomicTypeTag.Address])
    }
  ];
...
export class RotationCapability {
  ...
  static typeParameters: TypeParamDeclType[] = [];
  static fields: FieldDeclType[] = [{ name: 'account', typeTag: AtomicTypeTag.Address }];
...
export function accept_rotation_capability_ed25519_(
  ...
  rotation_capability = new RotationCapability(
    { account: $.copy(offerer_address) },
...
export function create_account_(new_address: HexString, $c: AptosDataCache): HexString {
  let temp$1;
...
export function create_account_unchecked_(new_address: HexString, $c: AptosDataCache): HexString {
    ...
    new Account(
      {
        ...
        rotation_capability_offer: new CapabilityOffer(
          ...
          new SimpleStructTag(CapabilityOffer, [new SimpleStructTag(RotationCapability)])
        ),
        signer_capability_offer: new CapabilityOffer(
          { for: Option.none_($c, [AtomicTypeTag.Address]) },
          new SimpleStructTag(CapabilityOffer, [new SimpleStructTag(SignerCapability)])
        )
      },
      new SimpleStructTag(Account)
    )
...
export function create_framework_reserved_account_(addr: HexString, $c: AptosDataCache): [HexString, SignerCapability] {
...
export function create_guid_(account_signer: HexString, $c: AptosDataCache): Guid.GUID {
  let account, addr;
...
export function create_resource_address_(source: HexString, seed: U8[], $c: AptosDataCache): HexString {
  let bytes;
...
export function create_signer_with_capability_(capability: SignerCapability, $c: AptosDataCache): HexString {
  let addr;
...
export function get_authentication_key_(addr: HexString, $c: AptosDataCache): U8[] {
  return $.copy($c.borrow_global<Account>(new SimpleStructTag(Account), $.copy(addr)).authentication_key);
}

export function get_guid_next_creation_num_(addr: HexString, $c: AptosDataCache): U64 {
...
export function get_signer_capability_address_(capability: SignerCapability, $c: AptosDataCache): HexString {
  return $.copy(capability.account);
}

export function increment_sequence_number_(addr: HexString, $c: AptosDataCache): void {
...
export function initialize_(aptos_framework: HexString, $c: AptosDataCache): void {
  ...
  $c.move_to(
    ...
    new OriginatingAddress(
      { address_map: Table.new___($c, [AtomicTypeTag.Address, AtomicTypeTag.Address]) },
      new SimpleStructTag(OriginatingAddress)
    )
  );
  return;
...
export function new_event_handle_(account: HexString, $c: AptosDataCache, $p: TypeTag[] /* <T>*/): Event.EventHandle {
...
export function offer_rotation_capability_ed25519_(
  ...
  let temp$1,
    ...
    auth_key,
    pubkey,
    rotation_capability_offer_proof_challenge,
  ...
  rotation_capability_offer_proof_challenge = new RotationCapabilityOfferProofChallenge(
    { sequence_number: $.copy(account_resource.sequence_number), recipient_address: $.copy(recipient_address) },
    new SimpleStructTag(RotationCapabilityOfferProofChallenge)
  );
  if (
  ...
  Option.fill_(account_resource.rotation_capability_offer.for__, $.copy(recipient_address), $c, [
...
export function buildPayload_offer_rotation_capability_ed25519(
  ...
  return $.buildPayload(
    ...
    typeParamStrings,
    [rotation_capability_sig_bytes, account_public_key_bytes, recipient_address],
    isJSON
  );
}
export function offer_signer_capability_(
  ...
  let temp$1,
    ...
    addr,
    expected_auth_key,
    expected_auth_key__3,
    proof_challenge,
    pubkey,
    pubkey__2,
    signer_capability_sig,
    signer_capability_sig__4;
  addr = Signer.address_of_(account, $c);
  if (exists_at_($.copy(addr), $c)) {
    temp$1 = exists_at_($.copy(recipient_address), $c);
  } else {
  ...
  account_resource = $c.borrow_global_mut<Account>(new SimpleStructTag(Account), $.copy(addr));
  proof_challenge = new SignerCapabilityOfferProofChallenge(
    { sequence_number: $.copy(account_resource.sequence_number), recipient_address: $.copy(recipient_address) },
    new SimpleStructTag(SignerCapabilityOfferProofChallenge)
  );
  if ($.copy(account_scheme).eq($.copy(ED25519_SCHEME))) {
    ...
    if (
      !Ed25519.signature_verify_strict_t_(signer_capability_sig, pubkey, proof_challenge, $c, [
        new SimpleStructTag(SignerCapabilityOfferProofChallenge)
      ])
    ) {
      throw $.abortCode(Error.invalid_argument_($.copy(EINVALID_PROOF_OF_KNOWLEDGE), $c));
    ...
    if ($.copy(account_scheme).eq($.copy(MULTI_ED25519_SCHEME))) {
      ...
      if (
        !Multi_ed25519.signature_verify_strict_t_(signer_capability_sig__4, pubkey__2, proof_challenge, $c, [
          new SimpleStructTag(SignerCapabilityOfferProofChallenge)
        ])
      ) {
        throw $.abortCode(Error.invalid_argument_($.copy(EINVALID_PROOF_OF_KNOWLEDGE), $c));
  ...
  if (Option.is_some_(account_resource.signer_capability_offer.for__, $c, [AtomicTypeTag.Address])) {
    Option.swap_(account_resource.signer_capability_offer.for__, $.copy(recipient_address), $c, [
      AtomicTypeTag.Address
    ]);
  } else {
    Option.fill_(account_resource.signer_capability_offer.for__, $.copy(recipient_address), $c, [
      AtomicTypeTag.Address
    ]);
  }
  return;
...
export function register_coin_(account_addr: HexString, $c: AptosDataCache, $p: TypeTag[] /* <CoinType>*/): void {
  let account;
  account = $c.borrow_global_mut<Account>(new SimpleStructTag(Account), $.copy(account_addr));
  Event.emit_event_(
    account.coin_register_events,
    new CoinRegisterEvent({ type_info: Type_info.type_of_($c, [$p[0]]) }, new SimpleStructTag(CoinRegisterEvent)),
    $c,
...
export function rotate_authentication_key_(
  ...
  challenge = new RotationProofChallenge(
    {
      ...
      current_auth_key: $.copy(curr_auth_key),
      new_public_key: $.copy(to_public_key_bytes)
    },
    new SimpleStructTag(RotationProofChallenge)
  );
  curr_auth_key__3 = verify_key_rotation_signature_and_get_auth_key_(
    $.copy(from_scheme),
  ...
  new_auth_key = verify_key_rotation_signature_and_get_auth_key_(
    $.copy(to_scheme),
  ...
  if (Table.contains_(temp$4, temp$5, $c, [AtomicTypeTag.Address, AtomicTypeTag.Address])) {
    if (
      !(
        $.copy(addr).hex() ===
        Table.remove_(address_map, $.copy(curr_address), $c, [AtomicTypeTag.Address, AtomicTypeTag.Address]).hex()
      )
    ) {
...
export function buildPayload_rotate_authentication_key(
  ...
  return $.buildPayload(
    ...
    typeParamStrings,
    [from_scheme, from_public_key_bytes, to_scheme, to_public_key_bytes, cap_rotate_key, cap_update_table],
    isJSON
  );
}
...
export function buildPayload_rotate_authentication_key_ed25519(
  ...
  return $.buildPayload(
    ...
    typeParamStrings,
    [curr_sig_bytes, new_sig_bytes, curr_pk_bytes, new_pk_bytes],
    isJSON
  );
}
export function rotate_authentication_key_internal_(account: HexString, new_auth_key: U8[], $c: AptosDataCache): void {
  let account_resource, addr;
  addr = Signer.address_of_(account, $c);
  if (!exists_at_($.copy(addr), $c)) {
...
export function verify_key_rotation_signature_and_get_auth_key_(
  ...
  if ($.copy(scheme).eq($.copy(ED25519_SCHEME))) {
    ...
    if (
      !Ed25519.signature_verify_strict_t_(sig, pk, $.copy(challenge), $c, [new SimpleStructTag(RotationProofChallenge)])
    ) {
      throw $.abortCode(Error.invalid_argument_($.copy(EINVALID_PROOF_OF_KNOWLEDGE), $c));
    }
    ...
    if ($.copy(scheme).eq($.copy(MULTI_ED25519_SCHEME))) {
      ...
      if (
        !Multi_ed25519.signature_verify_strict_t_(sig__2, pk__1, $.copy(challenge), $c, [
          new SimpleStructTag(RotationProofChallenge)
        ])
      ) {
        throw $.abortCode(Error.invalid_argument_($.copy(EINVALID_PROOF_OF_KNOWLEDGE), $c));
...
export class App {
  ...
  payload_offer_rotation_capability_ed25519(
    ...
    return buildPayload_offer_rotation_capability_ed25519(
  ...
  async offer_rotation_capability_ed25519(
    ...
    const payload = buildPayload_offer_rotation_capability_ed25519(
      rotation_capability_sig_bytes,
      account_public_key_bytes,
      recipient_address,
      _isJSON
  ...
  payload_rotate_authentication_key_ed25519(
    ...
    return buildPayload_rotate_authentication_key_ed25519(
      curr_sig_bytes,
      new_sig_bytes,
      curr_pk_bytes,
      new_pk_bytes,
  ...
  async rotate_authentication_key_ed25519(
    ...
    const payload = buildPayload_rotate_authentication_key_ed25519(
      curr_sig_bytes,
      new_sig_bytes,
      curr_pk_bytes,
      new_pk_bytes,
