  ...
  static fields: FieldDeclType[] = [
    ...
    { name: 'last_reconfiguration_time', typeTag: AtomicTypeTag.U64 },
    {
      ...
      typeTag: new StructTag(new HexString('0x1'), 'event', 'EventHandle', [
        new StructTag(new HexString('0x1'), 'reconfiguration', 'NewEpochEvent', [])
      ])
    }
  ];
...
export class NewEpochEvent {
  ...
  static typeParameters: TypeParamDeclType[] = [];
  static fields: FieldDeclType[] = [{ name: 'epoch', typeTag: AtomicTypeTag.U64 }];
...
export function disable_reconfiguration_(aptos_framework: HexString, $c: AptosDataCache): void {
...
export function emit_genesis_reconfiguration_event_($c: AptosDataCache): void {
  ...
  Event.emit_event_(
    config_ref.events,
    new NewEpochEvent({ epoch: $.copy(config_ref.epoch) }, new SimpleStructTag(NewEpochEvent)),
    $c,
...
export function enable_reconfiguration_(aptos_framework: HexString, $c: AptosDataCache): void {
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  if (!!reconfiguration_enabled_($c)) {
    throw $.abortCode(Error.invalid_state_($.copy(ECONFIGURATION), $c));
  }
    ...
    new Configuration(
      {
        ...
        events: Account.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(NewEpochEvent)])
      },
      new SimpleStructTag(Configuration)
    )
  );
...
export function last_reconfiguration_time_($c: AptosDataCache): U64 {
  return $.copy(
    $c.borrow_global<Configuration>(new SimpleStructTag(Configuration), new HexString('0x1')).last_reconfiguration_time
  );
}
