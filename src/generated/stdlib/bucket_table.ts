  ...
  static fields: FieldDeclType[] = [
    {
      ...
      typeTag: new StructTag(new HexString('0x1'), 'table_with_length', 'TableWithLength', [
        AtomicTypeTag.U64,
        new VectorTag(
          new StructTag(new HexString('0x1'), 'bucket_table', 'Entry', [new $.TypeParamIdx(0), new $.TypeParamIdx(1)])
        )
      ])
    },
...
export function add_(map: BucketTable, key: any, value: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): void {
  ...
  bucket = Table_with_length.borrow_mut_(map.buckets, $.copy(index), $c, [
    AtomicTypeTag.U64,
    new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
  ]);
  i = u64('0');
  ...
  Vector.push_back_(
    bucket,
    new Entry({ hash: $.copy(hash), key: key, value: value }, new SimpleStructTag(Entry, [$p[0], $p[1]])),
    $c,
    [new SimpleStructTag(Entry, [$p[0], $p[1]])]
  );
...
export function borrow_(map: BucketTable, key: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): any {
  let temp$1, temp$2, bucket, entry, i, index, len;
  index = bucket_index_(
    $.copy(map.level),
    $.copy(map.num_buckets),
    Aptos_hash.sip_hash_from_value_(key, $c, [$p[0]]),
    $c
  );
  bucket = Table_with_length.borrow_mut_(map.buckets, $.copy(index), $c, [
    AtomicTypeTag.U64,
...
export function borrow_mut_(map: BucketTable, key: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): any {
...
export function bucket_index_(level: U8, num_buckets: U64, hash: U64, $c: AptosDataCache): U64 {
  let temp$1, index;
...
export function contains_(map: BucketTable, key: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): boolean {
  let bucket, entry, i, index, len;
  index = bucket_index_(
    $.copy(map.level),
    $.copy(map.num_buckets),
  ...
  bucket = Table_with_length.borrow_(map.buckets, $.copy(index), $c, [
    AtomicTypeTag.U64,
    new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
  ]);
  i = u64('0');
...
export function destroy_empty_(map: BucketTable, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): void {
  let i;
  ...
  while ($.copy(i).lt($.copy(map.num_buckets))) {
    {
      Vector.destroy_empty_(
        Table_with_length.remove_(map.buckets, $.copy(i), $c, [
          AtomicTypeTag.U64,
          new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
        ]),
        $c,
  ...
  Table_with_length.destroy_empty_(buckets, $c, [
    AtomicTypeTag.U64,
    new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
  ]);
  return;
...
export function length_(map: BucketTable, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): U64 {
  return $.copy(map.len);
}

export function load_factor_(map: BucketTable, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): U64 {
  return $.copy(map.len)
    .mul(u64('100'))
    .div($.copy(map.num_buckets).mul($.copy(TARGET_LOAD_PER_BUCKET)));
}

export function new___(initial_buckets: U64, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): BucketTable {
  let buckets, map;
  if (!$.copy(initial_buckets).gt(u64('0'))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EZERO_CAPACITY), $c));
  }
  buckets = Table_with_length.new___($c, [
    AtomicTypeTag.U64,
    new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
  ]);
  Table_with_length.add_(buckets, u64('0'), Vector.empty_($c, [new SimpleStructTag(Entry, [$p[0], $p[1]])]), $c, [
    AtomicTypeTag.U64,
    new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
  ]);
  map = new BucketTable(
    { buckets: buckets, num_buckets: u64('1'), level: u8('0'), len: u64('0') },
    new SimpleStructTag(BucketTable, [$p[0], $p[1]])
  );
  split_(map, $.copy(initial_buckets).sub(u64('1')), $c, [$p[0], $p[1]]);
  return map;
}

export function remove_(map: BucketTable, key: any, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): any {
  let temp$1, temp$2, bucket, entry, i, index, len;
  index = bucket_index_(
    $.copy(map.level),
    $.copy(map.num_buckets),
...
export function split_(map: BucketTable, additional_buckets: U64, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): void {
...
export function split_one_bucket_(map: BucketTable, $c: AptosDataCache, $p: TypeTag[] /* <K, V>*/): void {
  ...
  if (
    $.copy(to_split)
      .add(u64('1'))
      .eq(u64('1').shl($.copy(map.level)))
  ) {
    map.level = $.copy(map.level).add(u8('1'));
  } else {
  }
  old_bucket = Table_with_length.borrow_mut_(map.buckets, $.copy(to_split), $c, [
    AtomicTypeTag.U64,
    new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
  ]);
  i = u64('0');
  ...
  Table_with_length.add_(map.buckets, $.copy(new_bucket_index), new_bucket, $c, [
    AtomicTypeTag.U64,
    new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]]))
  ]);
  return;
...
export function xor_(a: U64, b: U64, $c: AptosDataCache): U64 {
