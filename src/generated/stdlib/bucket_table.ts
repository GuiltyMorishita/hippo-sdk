  ...
  static fields: FieldDeclType[] = [
    {
      ...
      typeTag: new StructTag(
        ...
        [
          AtomicTypeTag.U64,
          new VectorTag(
            new StructTag(new HexString("0x1"), "bucket_table", "Entry", [
              new $.TypeParamIdx(0),
              new $.TypeParamIdx(1),
            ])
          ),
...
export function add_(
  ...
  Vector.push_back_(
    bucket,
    new Entry(
      { hash: $.copy(hash), key: key, value: value },
      new SimpleStructTag(Entry, [$p[0], $p[1]])
    ),
    $c,
    [new SimpleStructTag(Entry, [$p[0], $p[1]])]
...
export function borrow_mut_(
  ...
  while ($.copy(i).lt($.copy(len))) {
    {
      entry = Vector.borrow_mut_(bucket, $.copy(i), $c, [
        new SimpleStructTag(Entry, [$p[0], $p[1]]),
      ]);
      if ($.dyn_eq($p[0], entry.key, key)) {
        return entry.value;
...
export function contains_(
  ...
  while ($.copy(i).lt($.copy(len))) {
    {
      entry = Vector.borrow_(bucket, $.copy(i), $c, [
        new SimpleStructTag(Entry, [$p[0], $p[1]]),
      ]);
      if ($.dyn_eq($p[0], entry.key, key)) {
        return true;
...
export function new___(
  ...
  Table_with_length.add_(
    buckets,
    u64("0"),
    Vector.empty_($c, [new SimpleStructTag(Entry, [$p[0], $p[1]])]),
    $c,
    [
      AtomicTypeTag.U64,
      new VectorTag(new SimpleStructTag(Entry, [$p[0], $p[1]])),
  ...
  while ($.copy(i).lt($.copy(len))) {
    {
      ...
      if ($.dyn_eq($p[0], entry.key, key)) {
        let { value: value } = Vector.swap_remove_(bucket, $.copy(i), $c, [
          new SimpleStructTag(Entry, [$p[0], $p[1]]),
        ]);
...
export function split_one_bucket_(
  ...
  to_split = xor_(
    $.copy(new_bucket_index),
    u64("1").shl($.copy(map.level)),
    $c
  );
  ...
  old_bucket = Table_with_length.borrow_mut_(
    map.buckets,
    $.copy(to_split),
    $c,
    [
  ...
  while ($.copy(i).lt($.copy(j))) {
    {
      ...
      if ($.copy(index).eq($.copy(new_bucket_index))) {
        j = $.copy(j).sub(u64("1"));
        Vector.swap_(old_bucket, $.copy(i), $.copy(j), $c, [
          new SimpleStructTag(Entry, [$p[0], $p[1]]),
        ]);
      } else {
        i = $.copy(i).add(u64("1"));
  ...
  while ($.copy(j).lt($.copy(len))) {
    {
      entry__3 = Vector.pop_back_(old_bucket, $c, [
        new SimpleStructTag(Entry, [$p[0], $p[1]]),
      ]);
      Vector.push_back_(new_bucket, entry__3, $c, [
        new SimpleStructTag(Entry, [$p[0], $p[1]]),
      ]);
...
export function loadParsers(repo: AptosParserRepo) {
  repo.addParser(
    "0x1::bucket_table::BucketTable",
    BucketTable.BucketTableParser
  );
