...
export class AggregatorFactory {
  ...
  static fields: FieldDeclType[] = [
    {
      name: 'phantom_table',
      typeTag: new StructTag(new HexString('0x1'), 'table', 'Table', [AtomicTypeTag.Address, AtomicTypeTag.U128])
    }
  ];
...
export function create_aggregator_(account: HexString, limit: U128, $c: AptosDataCache): Aggregator.Aggregator {
  System_addresses.assert_aptos_framework_(account, $c);
  return create_aggregator_internal_($.copy(limit), $c);
}

export function create_aggregator_internal_(limit: U128, $c: AptosDataCache): Aggregator.Aggregator {
...
export function initialize_aggregator_factory_(aptos_framework: HexString, $c: AptosDataCache): void {
  let aggregator_factory;
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  aggregator_factory = new AggregatorFactory(
    { phantom_table: Table.new___($c, [AtomicTypeTag.Address, AtomicTypeTag.U128]) },
    new SimpleStructTag(AggregatorFactory)
  );
...
export class App {
  ...
  async loadAggregatorFactory(owner: HexString, loadFull = true, fillCache = true) {
