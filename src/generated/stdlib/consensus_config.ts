...
export class ConsensusConfig {
  ...
  static fields: FieldDeclType[] = [{ name: 'config', typeTag: new VectorTag(AtomicTypeTag.U8) }];
...
export function initialize_(aptos_framework: HexString, config: U8[], $c: AptosDataCache): void {
  ...
  $c.move_to(
    ...
    aptos_framework,
    new ConsensusConfig({ config: $.copy(config) }, new SimpleStructTag(ConsensusConfig))
  );
  return;
}

export function set_(account: HexString, config: U8[], $c: AptosDataCache): void {
  let config_ref;
...
export class App {
  ...
  async loadConsensusConfig(owner: HexString, loadFull = true, fillCache = true) {
