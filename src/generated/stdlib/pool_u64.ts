...
export function add_shares_(pool: Pool, shareholder: HexString, new_shares: U64, $c: AptosDataCache): U64 {
  ...
  if (contains_(temp$1, temp$2, $c)) {
    ...
    if ($.copy(new_shares).gt(u64('0'))) {
      ...
      Simple_map.add_(pool.shares, $.copy(shareholder), $.copy(new_shares), $c, [
        AtomicTypeTag.Address,
        AtomicTypeTag.U64
      ]);
...
export function amount_to_shares_(pool: Pool, coins_amount: U64, $c: AptosDataCache): U64 {
...
export function balance_(pool: Pool, shareholder: HexString, $c: AptosDataCache): U64 {
  let num_shares;
  num_shares = shares_(pool, $.copy(shareholder), $c);
  return shares_to_amount_(pool, $.copy(num_shares), $c);
}

export function buy_in_(pool: Pool, shareholder: HexString, coins_amount: U64, $c: AptosDataCache): U64 {
...
export function contains_(pool: Pool, shareholder: HexString, $c: AptosDataCache): boolean {
...
export function create_with_scaling_factor_(shareholders_limit: U64, scaling_factor: U64, $c: AptosDataCache): Pool {
  return new Pool(
    {
      shareholders_limit: $.copy(shareholders_limit),
      total_coins: u64('0'),
      total_shares: u64('0'),
      shares: Simple_map.create_($c, [AtomicTypeTag.Address, AtomicTypeTag.U64]),
...
export function deduct_shares_(pool: Pool, shareholder: HexString, num_shares: U64, $c: AptosDataCache): U64 {
...
export function multiply_then_divide_(pool: Pool, x: U64, y: U64, z: U64, $c: AptosDataCache): U64 {
  let result, scaling_factor;
  scaling_factor = to_u128_($.copy(pool.scaling_factor), $c);
  result = to_u128_($.copy(x), $c)
    .mul(to_u128_($.copy(y), $c))
    .mul($.copy(scaling_factor))
    .div(to_u128_($.copy(z), $c).mul($.copy(scaling_factor)));
  return u64($.copy(result));
}

export function redeem_shares_(pool: Pool, shareholder: HexString, shares_to_redeem: U64, $c: AptosDataCache): U64 {
...
export function shares_(pool: Pool, shareholder: HexString, $c: AptosDataCache): U64 {
  let temp$1;
...
export function shares_to_amount_(pool: Pool, shares: U64, $c: AptosDataCache): U64 {
  return shares_to_amount_with_total_coins_(pool, $.copy(shares), $.copy(pool.total_coins), $c);
}

export function shares_to_amount_with_total_coins_(pool: Pool, shares: U64, total_coins: U64, $c: AptosDataCache): U64 {
  let temp$1, temp$2;
  if ($.copy(pool.total_coins).eq(u64('0'))) {
    temp$1 = true;
  } else {
...
export function update_total_coins_(pool: Pool, new_total_coins: U64, $c: AptosDataCache): void {
