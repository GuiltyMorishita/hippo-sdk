...
export class UpgradePolicy {
  ...
  static typeParameters: TypeParamDeclType[] = [];
  static fields: FieldDeclType[] = [{ name: 'policy', typeTag: AtomicTypeTag.U8 }];
...
export function can_change_upgrade_policy_to_(from: UpgradePolicy, to: UpgradePolicy, $c: AptosDataCache): boolean {
  return $.copy(from.policy).le($.copy(to.policy));
}

export function check_coexistence_(old_pack: PackageMetadata, new_modules: String.String[], $c: AptosDataCache): void {
  ...
  while ($.copy(i).lt(Vector.length_(old_pack.modules, $c, [new SimpleStructTag(ModuleMetadata)]))) {
    {
      ...
      j = u64('0');
      while (
        $.copy(j).lt(Vector.length_(new_modules, $c, [new StructTag(new HexString('0x1'), 'string', 'String', [])]))
      ) {
        {
            ...
            while ($.copy(k).lt($.copy(r))) {
              {
                ...
                module_name__2 = $.copy(
                  Vector.borrow_(dep_pack.modules, $.copy(k), $c, [new SimpleStructTag(ModuleMetadata)]).name
                );
                Vector.push_back_(
                  allowed_module_deps,
                  new AllowedDep(
                    { account: $.copy(account__1), module_name: $.copy(module_name__2) },
                    new SimpleStructTag(AllowedDep)
                  ),
                  $c,
                  [new SimpleStructTag(AllowedDep)]
  ...
  while ($.copy(i).lt(Vector.length_(old_modules, $c, [new StructTag(new HexString('0x1'), 'string', 'String', [])]))) {
    {
      if (
        !Vector.contains_(
          new_modules,
          Vector.borrow_(old_modules, $.copy(i), $c, [new StructTag(new HexString('0x1'), 'string', 'String', [])]),
          $c,
          [new StructTag(new HexString('0x1'), 'string', 'String', [])]
        )
      ) {
...
export function get_module_names_(pack: PackageMetadata, $c: AptosDataCache): String.String[] {
  let i, module_names;
  ...
  while ($.copy(i).lt(Vector.length_(pack.modules, $c, [new SimpleStructTag(ModuleMetadata)]))) {
    {
      Vector.push_back_(
        module_names,
        $.copy(Vector.borrow_(pack.modules, $.copy(i), $c, [new SimpleStructTag(ModuleMetadata)]).name),
        $c,
        [new StructTag(new HexString('0x1'), 'string', 'String', [])]
      );
      i = $.copy(i).add(u64('1'));
...
export function initialize_(
  ...
  if (!$c.exists(new SimpleStructTag(PackageRegistry), $.copy(addr))) {
    $c.move_to(
      ...
      package_owner,
      new PackageRegistry({ packages: [metadata] }, new SimpleStructTag(PackageRegistry))
    );
  } else {
    Vector.push_back_(
      $c.borrow_global_mut<PackageRegistry>(new SimpleStructTag(PackageRegistry), $.copy(addr)).packages,
      metadata,
      $c,
...
export function is_policy_exempted_address_(addr: HexString, $c: AptosDataCache): boolean {
...
export function publish_package_txn_(
  ...
  return publish_package_(
    owner,
    Util.from_bytes_($.copy(metadata_serialized), $c, [new SimpleStructTag(PackageMetadata)]),
    $.copy(code),
    $c
  );
}
