...
export class TokenInfo 
{
  ...
  async loadFullState(app: $.AppType) {
    ...
    await this.description.loadFullState(app);
...
export class TokenRegistry 
{
  ...
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, TokenRegistry, typeParams);
  ...
  async loadFullState(app: $.AppType) {
    await this.symbol_to_token_info.loadFullState(app);
    await this.type_info_to_symbol.loadFullState(app);
    await this.symbol_to_list_idx.loadFullState(app);
    this.__app = app;
  }
...
export function add_token_ (
  ...
  registry = $c.borrow_global_mut<TokenRegistry>(new SimpleStructTag(TokenRegistry), $.copy(admin_addr));
  type_info = Aptos_std.Type_info.type_of_($c, [$p[0]]);
  token_info = new TokenInfo({ name: Std.String.utf8_($.copy(name), $c), symbol: Std.String.utf8_($.copy(symbol), $c), description: Std.String.utf8_($.copy(description), $c), decimals: $.copy(decimals), logo_url: Std.String.utf8_($.copy(logo_url), $c), project_url: Std.String.utf8_($.copy(project_url), $c), delisted: false, token_type: $.copy(type_info) }, new SimpleStructTag(TokenInfo));
  if (!!Aptos_std.Table.contains_(registry.symbol_to_token_info, $.copy(token_info.symbol), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new SimpleStructTag(TokenInfo)])) {
    throw $.abortCode($.copy(E_SYMBOL_ALREADY_EXISTS));
  }
  ...
  Aptos_std.Table.add_(registry.symbol_to_token_info, $.copy(token_info.symbol), $.copy(token_info), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new SimpleStructTag(TokenInfo)]);
  ...
  index = Std.Vector.length_(registry.token_info_list, $c, [new SimpleStructTag(TokenInfo)]);
  Std.Vector.push_back_(registry.token_info_list, $.copy(token_info), $c, [new SimpleStructTag(TokenInfo)]);
...
export function delist_token_ (
  ...
  if (!Aptos_std.Table.contains_(registry.symbol_to_token_info, $.copy(symbol_str), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new SimpleStructTag(TokenInfo)])) {
    throw $.abortCode($.copy(E_SYMBOL_DOES_NOT_EXIST));
  }
  ...
  type_info = $.copy(Aptos_std.Table.borrow_(registry.symbol_to_token_info, $.copy(symbol_str), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new SimpleStructTag(TokenInfo)]).token_type);
  Std.Vector.borrow_mut_(registry.token_info_list, $.copy(index), $c, [new SimpleStructTag(TokenInfo)]).delisted = true;
  ...
  Aptos_std.Table.remove_(registry.symbol_to_token_info, $.copy(symbol_str), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new SimpleStructTag(TokenInfo)]);
...
export function has_token_ (
  ...
  registry = $c.borrow_global<TokenRegistry>(new SimpleStructTag(TokenRegistry), $.copy(admin));
  type_info = Aptos_std.Type_info.type_of_($c, [$p[0]]);
...
export function initialize_ (
  ...
  admin_addr = Std.Signer.address_of_(admin, $c);
  if (!!$c.exists(new SimpleStructTag(TokenRegistry), $.copy(admin_addr))) {
    throw $.abortCode($.copy(E_ALREADY_INITIALIZED));
  }
  temp$6 = admin;
  temp$1 = $.copy(admin_addr);
  temp$2 = Aptos_std.Table.new___($c, [new StructTag(new HexString("0x1"), "string", "String", []), new SimpleStructTag(TokenInfo)]);
  ...
  temp$4 = Std.Vector.empty_($c, [new SimpleStructTag(TokenInfo)]);
  temp$5 = Aptos_std.Table.new___($c, [new StructTag(new HexString("0x1"), "string", "String", []), AtomicTypeTag.U64]);
  $c.move_to(new SimpleStructTag(TokenRegistry), temp$6, new TokenRegistry({ admin: temp$1, symbol_to_token_info: temp$2, type_info_to_symbol: temp$3, symbol_to_list_idx: temp$5, token_info_list: temp$4 }, new SimpleStructTag(TokenRegistry)));
  return;
}
...
export function is_registry_initialized_ (
  ...
  return $c.exists(new SimpleStructTag(TokenRegistry), $.copy(admin));
}
...
export function update_token_info_ (
  ...
  index = $.copy(Aptos_std.Table.borrow_(registry.symbol_to_list_idx, $.copy(symbol_str), $c, [new StructTag(new HexString("0x1"), "string", "String", []), AtomicTypeTag.U64]));
  list_token_info = Std.Vector.borrow_mut_(registry.token_info_list, $.copy(index), $c, [new SimpleStructTag(TokenInfo)]);
  ...
  table_token_info = Aptos_std.Table.borrow_mut_(registry.symbol_to_token_info, $.copy(symbol_str), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new SimpleStructTag(TokenInfo)]);
...
export class App {
  ...
  get TokenInfo() { return TokenInfo; }
  get TokenRegistry() { return TokenRegistry; }
  async loadTokenRegistry(
    owner: HexString,
    loadFull=true,
  ) {
    const val = await TokenRegistry.load(this.repo, this.client, owner, [] as TypeTag[]);
    if (loadFull) {
      await val.loadFullState(this);
    }
    return val;
  }
  payload_add_token_script(
    name: U8[],
    symbol: U8[],
    description: U8[],
    decimals: U8,
  ...
  async add_token_script(
    ...
    const payload = buildPayload_add_token_script(name, symbol, description, decimals, logo_url, project_url, $p);
    return $.sendPayloadTx(this.client, _account, payload, _maxGas);
  }
  payload_delist_token_script(
    symbol: U8[],
  ) {
    return buildPayload_delist_token_script(symbol);
  }
  async delist_token_script(
    _account: AptosAccount,
    symbol: U8[],
    _maxGas = 1000,
  ) {
    const payload = buildPayload_delist_token_script(symbol);
    return $.sendPayloadTx(this.client, _account, payload, _maxGas);
  }
  payload_initialize_script(
  ) {
    return buildPayload_initialize_script();
  }
  async initialize_script(
    _account: AptosAccount,
    _maxGas = 1000,
  ) {
    const payload = buildPayload_initialize_script();
    return $.sendPayloadTx(this.client, _account, payload, _maxGas);
  }
  payload_update_token_info_script(
    symbol: U8[],
    description: U8[],
    logo_url: U8[],
    project_url: U8[],
  ...
  async update_token_info_script(
    _account: AptosAccount,
    symbol: U8[],
    description: U8[],
    logo_url: U8[],
    ...
    const payload = buildPayload_update_token_info_script(symbol, description, logo_url, project_url);
    return $.sendPayloadTx(this.client, _account, payload, _maxGas);
  }
}
